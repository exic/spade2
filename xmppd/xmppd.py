...
globals()['MAXREQUESTLENGTH']
    = 10000  # maximum allowed length of XML-RPC request (in bytes)
globals()['XMPPD_MAX_CONNECTIONS']
    = 1  # [SOCKER] Set maximum number of connections for this node.

globals()['SOCKER_TGUID']
    = 'BBTECH_XMPPD'  # CHANGE THIS IF YOU ARE TO USE SOCKER!
    ...
    class poll:
        ...
        def poll(self, timeout=None):
            ...
            if timeout < 1 or timeout == None:
                pi, po, pe = select.select(poll['in'],
                     poll['out'], poll['err'])
            else:
                pi, po, pe = select.select(poll['in'], poll[
                    'out'], poll['err'], timeout / 1000.0)

            for x in poll['in']:
                if x in pi:
                    if data.has_key(x.fileno()) == True:
                        data[x.fileno()]['mask']
                            = data[x.fileno()]['mask'] | self.POLLIN
                    else:
            ...
            for x in poll['out']:
                if x in po:
                    if data.has_key(x.fileno()) == True:
                        data[x.fileno()]['mask']
                            = data[x.fileno()]['mask'] | self.POLLOUT
                    else:
            ...
            for x in poll['err']:
                if x in pe:
                    if data.has_key(x.fileno()) == True:
                        data[x.fileno()]['mask']
                            = data[x.fileno()]['mask'] | self.POLLERR
                    else:
...
class localizer:
    ...
    def localize(self, val, lang=None):
        if lang == None or lang in val == False:
            lang = self._lang
...
class Session:
    ...
    def push_queue(self, failreason=ERR_RECIPIENT_UNAVAILABLE):
        ...
        if self._stream_state>=STREAM__CLOSED or self._socket_state>=SOCKET_DEAD: # the stream failed. Return all stanzas that are still waiting for delivery.
            ...
            for key in self.deliver_key_queue:                            # Not sure. May be I
                self.dispatch(Error(self.deliver_queue_map[
                    key], failreason))  # should simply re-dispatch it?
            for stanza in self.stanza_queue:                              # But such action can invoke
                self.dispatch(Error(stanza, failreason))
                                       # Infinite loops in case of S2S connection...
            ...
            for stanza in self.stanza_queue:
                ...
                self.sendbuffer += txt
                self._stream_pos_queued += len(txt)
                           # should be re-evaluated for SSL connection.
                self.deliver_queue_map[self.
                    _stream_pos_queued]=stanza     # position of the stream when stanza will be successfully and fully sent
        ...
        if self.sendbuffer and select.select([], [self._sock], [])[1]:
            try:
                ...
                self.DEBUG('Attempting to kill %i!!!\n%s' % (
                    self._sock.fileno(), err), 'warn')
                # UNLOCK_QUEUE
                self.set_socket_state(SOCKET_DEAD)
                self.DEBUG(self._owner._l(SESSION_SEND_ERROR), 'error')
                return self.terminate_stream()
            self.DEBUG(repr(self._sock.fileno()) + ' ' + self.
                sendbuffer[:sent], 'sent')
        ...
        """
            ...
            if globals()['cmd_options'].enable_debug == True:
                 print 'starting-up libevent write-event for %i'%self._sock.fileno()
    ...
    def _catch_stream_id(self, ns=None, tag='stream', attrs={}):
        if not 'id' in attrs or not attrs['id']:
    ...
    def _stream_open(self, ns=None, tag='stream', attrs={}):
        ...
        if self.Stream.xmlns in [NS_CLIENT, NS_COMPONENT_ACCEPT]:
            ...
            xmlns = NS_SERVER
        text += ' xmlns:db="%s" xmlns:stream="%s" xmlns="%s"' % (
            NS_DIALBACK, NS_STREAMS, xmlns)
        if 'version' in attrs or self.TYP == 'client':
            text += ' version="1.0"'
        self.send(text + '>')
    ...
    def terminate_stream(self, error=None, unregister=1):
        ...
        if self.slave_session:
            self.slave_session.terminate_stream(
    ...
    def start_feature(self, f):
        if self.feature_in_process:
            raise "Starting feature %s over %s !" % (f, self.
                feature_in_process)
    ...
    def stop_feature(self, f):
        if self.feature_in_process != f:
            self.DEBUG(
            "Stopping feature %s instead of %s !"%(f,self.feature_in_process),'info')
        self.feature_in_process = None
    ...
    def set_session_state(self, newstate):
        if self._session_state < newstate:
            ...
            if split_jid != None and split_jid[0] in self._owner.administrators[self.ourname]:
                self.isAdmin = True
                self.DEBUG(self._owner._l(
                    SESSION_ADMIN_SET) % str(split_jid[0]), 'info')
...
class Socker_client:
    def __init__(self, owner, socker_host, tguid, sguid=None):
        ...
        if self.owner.cmd_options.setdefault('hostname', False) != None:
            self.owner.DEBUG('server',
                 "[SOCKER] hostname set to <%s>" % self.owner.cmd_options['hostname'],'info')
    ...
    def send_broadcast(self, stanza):
        if self.conn_okay == True:
            res = self._proxy.broadcast({'type_guid': self.
                _tguid+'_p%s'%str(self.fake_to_real_port[str(globals()['PORT_5222'])]),'server_guid':self._sguid,'stanza':stanza}) #Send broadcast to Socker
            if res['code'] != 1:
                return None
            else:
                return res
    ...
    def add_port(self, outside_port, host, port, options=None):
        self.onwer.DEBUG(
            'server' "[SOCKER] attempting to add route to Socker [%s]"%str((outside_port,host,port,options)),'info')
        if host == None and self._hostname != None:
            host = self._hostname
        elif host == None and self._hostname == None:
            host = self.get_hostname()

        self._chain[self._tguid + '_p%s' % str(outside_port)]
            = self._proxy.getchain({'type_guid':self._tguid+'_p%s'%str(outside_port)})
        if self._chain[self._tguid + '_p%s' % str(outside_port)]['code'] != 1:
            del self._chain[self._tguid + '_p%s' % str(outside_port)]

        inpt = {'outside_port': outside_port, 'type_guid': self.
            _tguid+'_p%s'%str(outside_port),'server_guid':self._sguid,'server_host':host,'server_port':port}
        if options != None:
            inpt.update(options)
    ...
    def add_data(self, data):
        if self.conn_okay == True:
            res = self._proxy.data({'act': 'add', 'type_guid':
                self._tguid+'_p%s'%str(self.fake_to_real_port[str(globals()['PORT_5222'])]),'server_guid':self._sguid,'pass':globals()['RPC_PASSWORD'],'add':data})
            if res['code'] == 1:
                return True
        return False
    ...
    def remove_data(self, data):
        if self.conn_okay == True:
            res = self._proxy.data({'act': 'remove', 'type_guid':
                self._tguid+'_p%s'%str(self.fake_to_real_port[str(globals()['PORT_5222'])]),'server_guid':self._sguid,'pass':globals()['RPC_PASSWORD'],'remove':data})
            if res['code'] == 1:
                return True
        return False
    ...
    def add_data_root(self, data):
        if self.conn_okay == True:
            res = self._proxy.data({'act': 'add', 'type_guid':
                self._tguid+'_p%s'%str(self.fake_to_real_port[str(globals()['PORT_5222'])]),'pass':globals()['RPC_PASSWORD'],'add':data})
            if res['code'] == 1:
                return True
        return False
    ...
    def remove_data_root(self, data):
        if self.conn_okay == True:
            res = self._proxy.data({'act': 'remove', 'type_guid':
                self._tguid+'_p%s'%str(self.fake_to_real_port[str(globals()['PORT_5222'])]),'pass':globals()['RPC_PASSWORD'],'remove':data})
            if res['code'] == 1:
                return True
        return False
    ...
    def destroy(self):
        for registered in self._registered:
            res = self._proxy.delete({'handle': registered[
                'handle'],'type_guid':self._tguid+'_p%s'%str(registered['outside_port']),'server_guid':self._sguid}) #Send request to Socker
    ...
    def RPC_handler(self, data):
        ...
        try:
            ...
            response = xmlrpclib.dumps(xmlrpclib.Fault(1,
                 "Socker(tm): %s" % traceback.format_exc()))

        if self.stage == XMLRPC_STAGE_TWO:
            final_output = ["HTTP/1.1 200 OK",
                "Server: BlueBridge Socker(tm)","Content-Length: %i"%len(response),"Connection: close","Content-Type: text/xml","",response]
            self.send('\n'.join(final_output))
            self.terminate()
...
#                print "sent",msg,sent
    ...
    def rpcexport_socker(self, inpt):
        ...
        if inpt['act'] == 'bc':
            try:
                self._owner.Dispatcher.dispatch(Node(node=
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        inpt['stanza']), Session_Dummy(self._owner))
            except:
                pass
            ...
            try:
                if inpt['time'] > time.time():
                    event.timeout(int(time.time() - inpt[
                        'time']), self._owner._lib_out)
                else:
...
class multisession_manager:
    ...
    class multisession_thread(threading.Thread):
        ...
        def unregistersession(self, s):
            ...
            self._owner.DEBUG('server', self._owner._l(
                SERVER_NODE_UNREGISTERED) % {'fileno': s.fileno(), 'raw': s})
            self.SESS_LOCK.release()
...
#        threading.Thread.__init__(self)
        ...
        try:
            ...
            cmd_options['select_enabled']
                = True  # If not, we'll have to just use the old select :/
        ...
        if not os.path.exists(self.cfgfile):
            self.DEBUG('server',
                'Could not load configuration file for xmppd. Bye', 'error')
        ...
        if not cmd_options.setdefault('password', None):
            globals()['RPC_PASSWORD'] = hashlib.sha1(str(time.
                time())+globals()['SOCKER_TGUID']+hashlib.sha1(str(time.time())).hexdigest()).hexdigest()
        else:
            globals()['RPC_PASSWORD'] = cmd_options['password']

        self.DEBUG('server',
            "[SECURITY] RPC_PASSWORD SET TO [%(pass)s]"%{'pass':globals()['RPC_PASSWORD']},'info')

        #Temp lang stuff
        globals()['DEFAULT_LANG'] = cmd_options.setdefault(
            'language', 'en')  # Changed to the --lang flag
...
#                    self.DEBUG('server','Plugging-out?','info')
        ...
        if cmd_options.setdefault('socker_info', None):
            self._socker = Socker_client(self, cmd_options[
                'socker_info'], globals()['SOCKER_TGUID'])
        if self._socker != None and self._socker.conn_okay == True:
            self.DEBUG('server',
                "[SOCKER] Socker(tm) support is enabled.", 'info')
            self.DEBUG(
                'server' "[SOCKER] Randomizing incoming connection ports.",'info')

            #Generate port map
            guide = [[globals()['PORT_5222'], self.pick_rand(),
                '5222'],[globals()['PORT_5223'],self.pick_rand(),'5223'],[globals()['PORT_5269'],self.pick_rand(),'5269'],[globals()['PORT_8000'],self.pick_rand(),'8000']]
            ...
            for port, new_port in port_map.iteritems():
                ...
                info = self._socker.add_port(int(port),
                    None,new_port,{'conn_max':globals()['XMPPD_MAX_CONNECTIONS'],'xmlrpc-callback':'http://%s:%i'%(self._socker._hostname,globals()['PORT_8000']),'pass':globals()['RPC_PASSWORD']})
                if info == None:
            ...
            if cmd_options.setdefault('socker_info', None):
                self.DEBUG('server',
                    "[SOCKER] Socker(tm) support could not be enabled. Please make sure that the Socker server is active.",'error')
            self._socker = None
        ...
        if cmd_options['select_enabled'] == True:
            ...
            reg_method = 'libevent'
            self.leventobjs[s.fileno()] = event.event(self.
                libevent_read_callback, handle = s, evtype = event.EV_TIMEOUT | event.EV_READ | event.EV_PERSIST)
    ...
    def tool_timeDurration(self, the_time):
        ...
        minutes = floor((the_time - days * 60 * 60 * 24 - hours *
             60 * 60) / 60)
        seconds = floor((the_time - days * 60 * 60 * 24 - hours * 60 * 60 -
             minutes * 60))
    ...
    def tool_get_status(self):
        ...
        data['uptime'] = self.tool_readableTimeDurration(time.time(
            ) - self.up_since)
    ...
    def registersession(self, s):
        ...
        if isinstance(self._socker, Socker_client):
            ...
            socker_notice = ''
        self.DEBUG('server', self._l(SERVER_NODE_REGISTERED) % {
            'fileno':s.fileno(),'raw':s,'method':reg_method,'socker_notice':socker_notice})
        self.SESS_LOCK.release()
    ...
    def unregistersession(self, s):
        ...
        del self.sockets[s.fileno()]  # Destroy the record
        self.DEBUG('server', self._l(SERVER_NODE_UNREGISTERED) % {
            'fileno': s.fileno(), 'raw': s})
        self.SESS_LOCK.release()
    ...
    def deactivatesession(self, peer):
        ...
        if peer in self.routes:
            del self.routes[peer]
        if peer in self.sisters:
...
#                    self.DEBUG('server','%s:%s is a server!'%(host,port),'info')
    ...
    def shutdown(self, reason):
        ...
        try:
            ...
            self.DEBUG('server',
                "Could not destroy multisession manager", "warn")
    ...
    def S2S(self,ourname,domain,slave_session=None,port=None,route_everything=False):
        ...
        #if s: return s
        ### THIS IS FUCKING DANGEROUS!!!!!
        s=Session(socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            , self, NS_SERVER, domain)
        s.slave_session = slave_session
        if route_everything == True:
    ...
    def _connect_session(self, session, domain, port):
        print session.DEBUG(self._l(SERVER_S2S_ATTEMPT_CONNECTION) %
            {'server': domain}, 'info')
        if port == None:
            port = 5269
        else:
        ...
        try:
            ...
            print session.DEBUG(self._l(
                SERVER_S2S_THREAD_ERROR) % err, 'error')
            self.num_servers -= 1
            ...
            session.terminate_stream(
                STREAM_REMOTE_CONNECTION_FAILED, unregister=0)
            return
    ...
    def Privacy(self, peer, stanza):
        ...
        template_input = {'jid_from': unicode(peer.peer).encode(
            'utf-8'), 'jid_to': unicode(stanza['to']).encode('utf-8')}
        split_jid = self.tool_split_jid(peer.peer)
        if split_jid == None:
            return
        self.DEBUG('server', self._l(SERVER_PVCY_ACCESS_CHECK) %
            template_input, 'info')
        ...
        if to_domain in self.servernames and to_domain != to:
            ...
            if name == 'presence':
                if stanza.getType() in ["subscribe", "subscribed", "unsubscribe", "unsubscribed"]:
                    self.DEBUG('server', self._l(
                        SERVER_PVCY_ACCESS_CLEAR_ONEWAY_PRESENCE)%template_input,'info')
                    return

            if node + '@' + domain == bareto:
                self.DEBUG('server', self._l(
                    SERVER_PVCY_ACCESS_CLEAR_UNLIMITED) %template_input, 'info')
                return
            ...
            if to_working_roster_item == None and anon_allow == False:
                ...
                self.DEBUG('server', self._l(
                    SERVER_PVCY_ACCESS_NOTCLEAR_DOUBLEFALSE)%template_input,'error')
            ...
            for z, a in roster.iteritems():
                if z == bareto:
                    if a['subscription']=='both' and to_working_roster_item.has_key('subscription') and to_working_roster_item['subscription']=='both':
                        self.DEBUG('server', self.
                            _l(SERVER_PVCY_ACCESS_CLEAR_BIDIRECTIONAL)%template_input,'info')
                        return
                    elif to_working_roster_item['subscription'] == 'from':
                        self.DEBUG('server', self.
                            _l(SERVER_PVCY_ACCESS_CLEAR_ONEWAY)%template_input,'info')
                        return
                    elif to_working_roster_item['subscription'] == 'to':
                        peer.send(Error(stanza, ERR_NOT_AUTHORIZED))
                        self.DEBUG('server', self.
                            _l(SERVER_PVCY_ACCESS_NOTCLEAR_MODETO)%template_input,'error')
                        raise NodeProcessed  # Take the blue pill
            ...
            if anon_allow == True or str(to) in self.servernames:
                ...
                peer.send(Error(stanza, ERR_NOT_AUTHORIZED))
                self.DEBUG('server', self._l(
                    SERVER_PVCY_ACCESS_NOTCLEAR_FALSEANON)%template_input,'error')
                raise NodeProcessed  # Take the blue pill
        return
    ...
    def run(self):
        ...
        while GLOBAL_TERMINATE == False:
            ...
            if the_input == 'restart':
                self._owner.DEBUG('server',
                    'Stand-by; Restarting entire server!', 'info')
                self._owner.shutdown(STREAM_SYSTEM_SHUTDOWN)
                self._owner.DEBUG('server',
                    'Server has been shutdown, restarting NOW!', 'info')
                GLOBAL_TERMINATE = False
            ...
            #                    self.DEBUG('server','Plugging-out?','info')
                                ...
                                if addon(
                                    ).__class__.__dict__.has_key('plugout'): addon().plugout()
            ...
            #                    self.DEBUG('server','Plugging-out?','info')
                        ...
                        if self._owner.__dict__.has_key(addon().__class__.__name__) == True:
                            self._owner.DEBUG(
                                'server','Error: Could not un-plug %s'%addon().__class__.__name__,'error')
                        self._owner.unfeature(addon.NS)
...
if __name__ == '__main__':
    ...
    parser = OptionParser(usage=
                                                                                                                                                                                                                                                                                                                                                                                                                "%prog [options] [-l lang_code] [--hostname=HOST] [-s host[:ip]]",version="%%prog %s"%__version__)
...
#    s=Server(cmd_options=eval(str(cmd_options)))
    ...
    if cmd_options.enable_interactive == True:
        inpt_service.start()
...
#            s.shutdown(STREAM_SYSTEM_SHUTDOWN)
            ...
            tbfd.close()
        if cmd_options.disable_fallback == True:
            GLOBAL_TERMINATE = True
