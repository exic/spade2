...
class Dialback(PlugIn):
    ...
    def plugin(self, server):
        server.Dispatcher.RegisterHandler('result', self.
            dialbackHandler, xmlns=NS_DIALBACK)
        server.Dispatcher.RegisterHandler('verify', self.
            dialbackHandler, xmlns=NS_DIALBACK)
    ...
    def dialbackHandler(self, session, stanza):
        ...
        if to not in self._owner.servernames:
            ...
            key = stanza.getData()
            self.DEBUG('Received dialback key %s (%s->%s).' % (`
                key`, frm, to), 'info')
            # Now we should form a request and send it to authoritative server
            req = Node('db: verify', {'from': session.ourname, 'to':
                frm, 'id': session.ID}, [key])
            s = self._owner.getsession(frm)
            if not s:
                print "### db:No session for " + str(frm)
                s = self._owner.S2S(session.ourname, frm.
                    getDomain(), slave_session=session)
            s.send(req)
            ...
            key = stanza.getData()
            self.DEBUG(
                'Received dialback key %s for verification against id %s.'%(key,id),'info')
            if key.strip() == hashlib.sha1(id + self._owner.
                ID).hexdigest():
                    typ = 'valid'
            else:
                typ = 'invalid'
            rep = Node('db: verify', {'from': to, 'to': frm,
                 'id': id, 'type': typ})
            session.send(rep)
        elif name == 'verify' and session.TYP == 'client':
            # (9) Received the verification reply
            self.DEBUG(
                'Received verified dialback key for id %s (%s->%s). Result is: %s.'%(stanza['id'],frm,to,stanza['type']),'info')
            if frm in self.waitlist:
                key, s = self.waitlist[frm]
                if s.ID == stanza['id']:
                    rep = Node('db: result', {'from': to,
                        'to': frm, 'type': stanza['type']})
                    s.send(rep)
                    ...
                    session.terminate_stream(
                        STREAM_INVALID_ID) # it will terminate "initial stream" also
    ...
    def __call__(self, session):
        ...
        req = Node('db: result', {'from': session.ourname, 'to':
            session.peer}, [key])
        session.send(req)
