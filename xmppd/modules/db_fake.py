...
def build_database(server_instance):
    ...
    for a_registered_server in server_instance.servernames:
        server_instance.DEBUG('server',
            'DB: Building database tree for %s' % a_registered_server, 'info')
        if not a_registered_server in db:
            db[a_registered_server] = {}

        db[a_registered_server]['__ir__'] = {}
        db[a_registered_server]['__ir__']['storage'] = {'karma': {
            'down':307200,'up':307200307200307200,'last_time':0.0,'tot_down':0,'tot_up':0}}
        ...
        db[a_registered_server]['test']['storage'] = {'karma':{
            'down':307200,'up':307200307200307200,'last_time':0.0,'tot_down':0,'tot_up':0}}
        ...
        db[a_registered_server]['test']['groups']['Friends'] = [
            'test2@172.16.1.34','test3@172.16.1.34']

        db[a_registered_server]['test2'] = {}
        db[a_registered_server]['test2']['storage'] = {'karma':{
            'down':307200,'up':307200,'last_time':0.0,'tot_down':0,'tot_up':0}}
        db[a_registered_server]['test2']['password'] = 'test'
        db[a_registered_server]['test2']['anon_allow'] = 'yes'

        db[a_registered_server]['test2']['roster'] = {}
        db[a_registered_server]['test2']['roster']['test3@'+
            a_registered_server] = {'subscription':'both'}

        db[a_registered_server]['test2']['groups'] = {}
        db[a_registered_server]['test2']['groups']['Friends'] = [
            'test3@172.16.1.34','test3@172.16.0.2','test3@'+a_registered_server]

        db[a_registered_server]['test3'] = {}
        db[a_registered_server]['test3']['storage'] = {'karma':{
            'down':307200,'up':307200,'last_time':0.0,'tot_down':0,'tot_up':0}}
        ...
        db[a_registered_server]['test3']['roster']['test2@'+
            a_registered_server] = {'subscription':'both'}
        db[a_registered_server]['test3']['roster']['pixelcort@'+
            a_registered_server] = {'subscription':'both'}
        db[a_registered_server]['test3']['roster']['kris_tate@'+
            a_registered_server] = {'subscription':'both'}

        ##Item Groups
        db[a_registered_server]['test3']['groups'] = {}
        db[a_registered_server]['test3']['groups'][
            'かっこういいな人'] = ['test2@172.16.1.34','test2@172.16.0.2','test2@'+a_registered_server,'pixelcort@'+a_registered_server,'kris_tate@'+a_registered_server]

        db[a_registered_server]['pixelcort'] = {}
        db[a_registered_server]['pixelcort']['storage'] = {'karma':
            {'down':307200,'up':307200,'last_time':0.0,'tot_down':0,'tot_up':0}}
        ...
        db[a_registered_server]['pixelcort']['roster']['tekcor@'+
            a_registered_server] = {'subscription':'both'}
        db[a_registered_server]['pixelcort']['roster'][
            'kris_tate@'+a_registered_server] = {'subscription':'both'}
        db[a_registered_server]['pixelcort']['roster']['mvanveen@'+
            a_registered_server] = {'subscription':'both'}

        ##Item Groups
        db[a_registered_server]['pixelcort']['groups'] = {}
        db[a_registered_server]['pixelcort']['groups']['Friends']
             = ['tekcor@'+a_registered_server,'mvanveen@'+a_registered_server]
        db[a_registered_server]['pixelcort']['groups']['Kris'] = [
            'kris_tate@'+a_registered_server]

        db[a_registered_server]['kris_tate'] = {}
        db[a_registered_server]['kris_tate']['storage'] = {'karma':
            {'down':307200,'up':1000,'last_time':0.0,'tot_down':0,'tot_up':0}}
        ...
        db[a_registered_server]['kris_tate']['roster']['tekcor@'+
            a_registered_server] = {'subscription':'both'}
        db[a_registered_server]['kris_tate']['roster'][
            'pixelcort@'+a_registered_server] = {'subscription':'both'}
        db[a_registered_server]['kris_tate']['roster']['mvanveen@'+
            a_registered_server] = {'subscription':'both'}

        ##Item Groups
        db[a_registered_server]['kris_tate']['groups'] = {}
        db[a_registered_server]['kris_tate']['groups'][
            'かっこういいな人'] = ['tekcor@'+a_registered_server,'pixelcort@'+a_registered_server,'mvanveen@'+a_registered_server]

        db[a_registered_server]['tekcor'] = {}
        db[a_registered_server]['tekcor']['storage'] = {'karma':{
            'down':307200,'up':307200,'last_time':0.0,'tot_down':0,'tot_up':0}}
        ...
        db[a_registered_server]['tekcor']['roster']['pixelcort@'+
            a_registered_server] = {'subscription':'both'}
        db[a_registered_server]['tekcor']['roster']['kris_tate@'+
            a_registered_server] = {'subscription':'both'}

        ##Item Groups
        db[a_registered_server]['tekcor']['groups'] = {}
        db[a_registered_server]['tekcor']['groups']['Friends'] = [
            'kris_tate@'+a_registered_server,'pixelcort@'+a_registered_server]


        db[a_registered_server]['mvanveen'] = {}
        db[a_registered_server]['mvanveen']['storage'] = {'karma':{
            'down':307200,'up':307200,'last_time':0.0,'tot_down':0,'tot_up':0}}
        ...
        db[a_registered_server]['mvanveen']['roster']['pixelcort@'+
            a_registered_server] = {'subscription':'both'}
        db[a_registered_server]['mvanveen']['roster']['kris_tate@'+
            a_registered_server] = {'subscription':'both'}

        ##Item Groups
        db[a_registered_server]['mvanveen']['groups'] = {}
        db[a_registered_server]['mvanveen']['groups']['Friends']
             = ['kris_tate@'+a_registered_server,'pixelcort@'+a_registered_server]
        """
...
class DB(PlugIn):
    ...
    def save_to_roster(self, domain, node, jid, info, add_only_if_already=False):
        if not node:
            node, domain = domain.split(".", 1)
        self.DEBUG(
            "Saving roster info to database %s-->(%s) [%s]:\n"%(jid,node+'@'+domain,str(info)),'info')
    ...
    def pull_roster(self, domain, node, jid):
        ...
        try:
            ...
            if 'subscription' in data == False:
                data.update({'subscription': 'none'})
            return data
        except KeyError:
            self.DEBUG(
                'DB ERR: Could not retrieve %s@%s roster for %s'%(node,domain,jid),'warn')
            return None
    ...
    def del_from_roster(self, domain, node, jid):
        if not node:
            node, domain = domain.split(".", 1)
        self.DEBUG(
            "Deleting roster info from database %s--X(%s):\n"%(jid,node+'@'+domain),'info')
        try:
            ...
            self.DEBUG(
                'DB ERR: A Client tried to remove a contact that wasn\'t even added! (%s::%s::%s)'%(domain,node,jid),'warn')
            return False
    ...
    def del_from_roster_jid(self, domain, node, jid, what):
        ...
        try:
            ...
            self.DEBUG(
                'DB ERR: A Client tried to remove a contact attr that wasn\'t even added! (%s::%s::%s)'%(domain,node,jid),'warn')
            return False
    ...
    def del_groupie(self, domain, node, jid):
        ...
        try:
            self.DEBUG(
                "Deleting groupie from database %s--X(%s):\n"%(jid,node+'@'+domain),'info')
            for gn, gm in db[domain][node]['groups'].iteritems():
                if jid in db[domain][node]['groups'][gn]:
                    db[domain][node]['groups'][gn].remove(jid)
        except Exception, err:
            self.DEBUG(
                'DB ERR: A groupie went mad! %s::%s::%s'%(domain,node,jid),'warn')
        self.save_database()
    ...
    def get(self, domain, node, what):
        ...
        try:
            ...
            self.DEBUG('DB ERR: Could not retrieve %s::%s::%s' %
                (domain, node, what), 'warn')
            return None
    ...
    def delete(self, domain, node, what):
        ...
        try:
            ...
            self.DEBUG('DB ERR: Could not delete %s::%s::%s' % (
                domain, node, what), 'warn')
            return None
    ...
    def register_user(self, domain, node, password, name):
        try:
            ...
            self.DEBUG("Registered user %s in domain %s" % (node,
                 domain), 'info')
            self.save_database()
            return True
        except:
            self.DEBUG(
                'Error registering username %s in domain %s'%(node,domain),'error')
            return False
