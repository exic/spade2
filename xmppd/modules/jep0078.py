...
class NSA(PlugIn):
    ...
    def plugin(self, server):
        server.Dispatcher.RegisterHandler('iq', self.
            getAuthInfoHandler, 'get', NS_AUTH)
        server.Dispatcher.RegisterHandler('iq', self.
            setAuthInfoHandler, 'set', NS_AUTH)
    ...
    def setAuthInfoHandler(self, session, stanza):
        ...
        if password is not None:
            digest = hashlib.sha1(session.ID +
            password).hexdigest()
        try:
        ...
        if servername not in self._owner.servernames:
          ...
          and stanza.T.query.T.resource.getData():
            ...
            fulljid = "%s@%s/%s" % (username, servername, stanza.T.
                query.T.resource.getData())
