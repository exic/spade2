...
class ROSTER(PlugIn):
    ...
    def plugin(self, server):
        server.Dispatcher.RegisterHandler('iq', self.
            RosterIqHandler, typ='set', ns=NS_ROSTER, xmlns=NS_CLIENT)
        server.Dispatcher.RegisterHandler('iq', self.
            RosterIqHandler, typ='get', ns=NS_ROSTER, xmlns=NS_CLIENT)
            ...
            for kid in stanza.getTag('query').kids:
                ...
                if kid.getName() == 'item' and kid.getAttr('subscription') != 'remove':
                    ...
                    if subscription != None:
                        ...
                        self.DEBUG(
                            '###ROSTER+: Wow, subscription is not active -- better create one pronto!','warn')
                        #kid.setAttr('subscription','none')
                        info.update({'subscription': 'none'})

                    ask = kid.getAttr('ask')
                    if ask != None or ask_subscribe == True:
                        info.update({'ask': ask})
                    ...
                    self._owner.DB.save_to_roster(
                        s_split_jid[1],s_split_jid[0],split_jid[0]+'@'+split_jid[1],info)
                    if kid.kids != []:
                        group_list = []
    ...
    def RosterRemove(self, session, stanza):
        ...
        if stanza.getType() == 'set' and stanza.getTag('query').kids != []:
            for kid in stanza.getTag('query').kids:
                if kid.getName() == 'item' and kid.getAttr('subscription') == 'remove':
                    #split_jid = self._owner.tool_split_jid(kid.getAttr('jid'))
                    p = Presence(to=kid.getAttr('jid'),
                        frm=session.getBareJID(), typ='unsubscribe')
                    session.dispatch(p)
                    split_jid = self._owner.tool_split_jid(kid.getAttr('jid'))
                    p = Presence(to=kid.getAttr('jid'),
                        frm=session.getBareJID(), typ='unsubscribed')
                    session.dispatch(p)

                    session.enqueue(stanza)

                    self._owner.DB.del_from_roster(
                        s_split_jid[1], s_split_jid[0], kid.getAttr('jid'))
                    self._owner.DB.del_groupie(
                        s_split_jid[1], s_split_jid[0], kid.getAttr('jid'))

                    #Tell 'em we just road-off into the sunset
                    split_jid = self._owner.tool_split_jid(kid.getAttr('jid'))
                    p = Presence(to=kid.getAttr('jid'),
                        frm=session.peer, typ='unavailable')
                    session.dispatch(p)
    ...
    def RosterPushOneToClient(self, contact, to, to_session=None, mode='set',options=None):
        ...
        if not self._owner.Router.isFromOutside(to_domain):
            ...
            item = self._owner.DB.pull_roster(to_domain,
                 to_node, str(contact))
            """
            <iq type='set'>
            ...
            try:
                ...
                pass
        self.DEBUG(
            '#ROSTER#: Pushing one out to client %s! [COMPLETE]'%(barejid),'warn')
        ...
        if not self._owner.Router.isFromOutside(to_domain):
            ...
            if options == {} and 'attr' in options:
                for ok, od in options['attr']:
                    atag.setAttr(ok, od)
            if atag.getAttr('name') == None and name != None:
                atag.setAttr('name', name)

            ask = atag.getAttr('ask')
            if ask == 'InternalDelete':
                atag.delAttr('ask')
                self._owner.DB.del_from_roster_jid(
                    s_split_jid[1], s_split_jid[0], bareto, 'ask')
