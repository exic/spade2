...
class Config(PlugIn):
    ...
    def plugin(self, server):
        ...
        for name in servernames:
            server.DEBUG('server',
                'Added new server (%s) from config!' % name.split(), 'info')
        ...
        for name, v in administrators.items():
            for admin in v:
                ...
                server.DEBUG('server',
                    'Added new admin (%s) for server (%s) from config!'%(admin.strip(),name),'info')

        server.plugins = copy.copy(plugins)
        server.DEBUG('server', 'Added plugins list: %s' % (str(server.
            plugins)), 'ok')
        server.components = components
        server.DEBUG('server', 'Added components list: ' + str(server.
            components), 'ok')
