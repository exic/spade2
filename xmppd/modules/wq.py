...
class Group:
    ...
    def __init__(self, gname, wq, users=[]):
        ...
        self.users = dict()
        self.agents = dict()
              # An "agent" here is a client that "works" in the group and assists users
        ...
        if join_queue:
            ...
            if ns == "http://jabber.org/protocol/workgroup" and typ == 'set' and queue_notifications:
                ...
                try:
                    ...
                    self.DEBUG(
                        "User tried to join the same group again", "warn")
                    ...
                    self.DEBUG(
                        "Unknown exception when user was joining a group","error")
                    return
        elif depart_queue:
            ...
            if ns == "http://jabber.org/protocol/workgroup" and typ == 'set':
                ...
                try:
                    if self.delUser(user):
                        ...
                        dq = m.setTag(
                            "depart-queue", {"xmlns":"http://jabber.org/protocol/workgroup"})
                        session.enqueue(m)
                        return
                except ItemNotFound:
                    ...
                    self.DEBUG(
                        "Unknown exception when user was departing a group","error")
                    return
        elif queue_status:
    ...
    def addUser(self, barejid):
        if barejid not in self.users.keys():
            ...
            self.DEBUG("User %s joined group %s" % (barejid,
                 self.getName()), "ok")
            return True
        else:
            raise Conflict
    ...
    def delUser(self, barejid):
        if barejid in self.users.keys():
            ...
            self.DEBUG("User %s departed group %s" % (barejid,
                self.getName()), "ok")
            return True
        else:
            raise ItemNotFound
    ...
    def updateAgent(self, barejid, pres):
        ...
        self.DEBUG("Agent %s joined group %s" % (barejid, self.
            getName()), "ok")
        return True
...
class WQ(PlugIn):
    ...
    def plugin(self, server):
        ...
        self.DEBUG("Created WQ: '%s' '%s'" % (self.name, str(self.
            jid)), "warn")
    ...
    def addGroup(self, group, name=""):
        if not group and not name:
            ...
            if group.getName() in self.groups.keys():
                ...
                self.DEBUG(
                    "Added group %s to Workgroup Queues"%(group.getName()),"ok")
                return True
