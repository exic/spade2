...
class TLS(PlugIn):
    ...
    def plugin(self, server):
        server.Dispatcher.RegisterHandler('starttls', self.
            starttlsHandler, xmlns=NS_TLS)
        server.Dispatcher.RegisterHandler('proceed', self.
            proceedfailureHandler, xmlns=NS_TLS)
        server.Dispatcher.RegisterHandler('failure', self.
            proceedfailureHandler, xmlns=NS_TLS)
        server.Dispatcher.RegisterHandler('features', self.
            FeaturesHandler, xmlns=NS_STREAMS)
    ...
    def starttlsHandler(self, session, stanza):
        ...
        if not certfile or not keyfile:
            session.send(Node('failure', {'xmlns': NS_TLS}))
            self.DEBUG(
                'TLS startup failure: can\'t find SSL cert/key file[s].','error')
            session.unfeature(NS_TLS)    # do not declare TLS anymore
            session.stop_feature(NS_TLS)
                 # TLS finished, let another features start
        else:
    ...
    def FeaturesHandler(self, session, stanza):
        ...
        if not stanza.getTag('starttls', namespace=NS_TLS):
            self.DEBUG(
                "TLS unsupported by remote server; Doing nothing -- Prob. jabber.org?",'warn')
            return
        else:
            self.DEBUG(
                "TLS supported by remote server. Requesting TLS start.", 'ok')
...
#        server.Dispatcher.RegisterHandler('features',self.FeaturesHandler,xmlns=NS_STREAMS)
        self.mechanisms = ['PLAIN']
            #,'DIGEST-MD5']  # for announce in <features/> tag
    ...
    def FeaturesHandler(self,session,feats):
        ...
        if "DIGEST-MD5" in mecs:
            ...
            sasl_data='%s\x00%s\x00%s'%(self.username+'@'+
                session.peer,self.username,self.password)
            node=Node('auth',attrs={'xmlns':NS_SASL,
                'mechanism':'PLAIN'},payload=[base64.encodestring(sasl_data)])
        else:
            session.startsasl='failure'
            self.DEBUG(
                'I can only use DIGEST-MD5 and PLAIN mecanisms.','error')
            return
...
"""
    ...
    def reject_auth(self, session, authzid='unknown'):
        session.send(Node('failure', {'xmlns': NS_SASL}, [Node(
            'not-authorized')]))
    ...
    def SASLHandler(self, session, stanza):
        ...
        if NS_SASL in session.features:
            ...
            raise NodeProcessed
        if not 'sasl' in session.__dict__:
            session.sasl = {'retries': 3}
        if not 'next' in session.sasl:
        ...
        if stanza.getName() == 'auth':
            ...
            if mec == 'PLAIN':
                ...
                session.terminate_stream(Node('failure', {
                    'xmlns': NS_SASL}, [Node('invalid-mechanism')]))
            raise NodeProcessed
            ...
            if chal.has_key('qop') and chal['qop']=='auth':
                ...
                A1=C([H(C([resp['username'],resp['realm'],
                    self.password])),resp['nonce'],resp['cnonce']])
                A2=C(['AUTHENTICATE',resp['digest-uri']])
                response= HH(C([HH(A1),resp['nonce'],resp[
                    'nc'],resp['cnonce'],resp['qop'],HH(A2)]))
                resp['response']=response
                resp['charset']='utf-8'
                sasl_data=''
                for key in ['charset','username','realm','nonce','nc','cnonce','digest-uri','response','qop']:
                    if key in ['nc','qop','response',
                        'charset']: sasl_data+="%s=%s,"%(key,resp[key])
                    else: sasl_data+='%s="%s",'%(key,resp[key])
                node=Node('response',attrs={'xmlns':
                    NS_SASL},payload=[base64.encodestring(sasl_data[:-1]).replace('\n','')])
...
class Bind(PlugIn):
    ...
    def plugin(self, server):
        server.Dispatcher.RegisterHandler('iq', self.bindHandler,
            typ='set', ns=NS_BIND, xmlns=NS_CLIENT)
    ...
    def bindHandler(self, session, stanza):
        if session.xmlns != NS_CLIENT or 'resource' in session.__dict__:
            ...
            if session._session_state < SESSION_AUTHED:
                ...
                raise NodeProcessed
            resource = stanza.getTag('bind', namespace=NS_BIND)
                .T.resource.getData()
            if not resource:
                resource = session.ID
...
class Session(PlugIn):
    ...
    def plugin(self, server):
        server.Dispatcher.RegisterHandler('iq', self.sessionHandler,
            typ='set', ns=NS_SESSION, xmlns=NS_CLIENT)
...
class Handshake(PlugIn):
    ...
    def plugin(self, server):
        self.server = server
        server.Dispatcher.RegisterNamespaceHandler(
            NS_COMPONENT_ACCEPT, self.namespaceChangerAndRedirector)
    ...
    def handshakeHandler(self, session, stanza):
        ...
        for k, v in self.server.components.items():
            try:
                truehs = hashlib.sha1(str(session.ID) + v[
                    'password']).hexdigest()
                if handshake == truehs:
                    ...
                    self.server.DB.register_user(dom,
                         node, v['password'], v['name'])
