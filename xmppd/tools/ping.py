...
interation_max = int(raw_input(
    "How many times will we be grilling the server?\n"))
...
try:
    while i < interation_max:
        ...
        print len(data)
        message_pool.update({data: {'start': the_time, 'fin': 0.0,
            'returned': False}})
...
for x, y in message_pool.iteritems():
    if message_pool[x]['returned'] == True:
