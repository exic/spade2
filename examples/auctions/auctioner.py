            ...
            try:
                for key in self.server.behav.myAgent.auctions.keys():
                    ...
                    if self.server.behav.myAgent.auctions[key]["current_winner"]:
                        s = s + str(self.server.
                            behav.myAgent.auctions[key]["current_winner"]) + ";;"
                    else:
                        s = s + "-;;"
                print s
                self.wfile.write(s)
...
class Auctioner(Agent.Agent):
        ...
        def _process(self):
            ...
            if "BID" in msg.getContent():
                ...
                # Check wether the auction is up
                    ...
                    # Check if the bidder is already the highest bidder
                        ...
                        print "The new price is:",
                            str(self.myAgent.
                                auctions[auction]['price']), "made by", str(msg.getSender().getName())
                        #rep.setContent(msg.getContent())
                        rep.setContent(
                            "CURRENTBID "+str(auction)+" "+str(bid)+" "+str(msg.getSender().getName()))
                        self.myAgent.auctions[
                            auction]["current_winner"] = msg.getSender().getName()
                ...
                for k, auction in self.myAgent.auctions.items():
                    if auction["countdown"]:
                        cont = cont + str(k)
                            + "::" # + str(auction["name"]) + " " + str(auction["desc"]) + ""
                ...
                if auction['time'] > 0 and auction["countdown"]:
                    ...
                    if auction['time'] <= 0:
                        ...
                        msg.setContent("WINNER " +
                            str(key)+" "+str(auction["price"])+" "+str(auction["current_winner"]))
    ...
    def _setup(self):
        self.auctions = {}
        self.auctions["WII"] = {'price': 10, 'name': 'Wii', 'desc':
            'Nintendo Wii Game Console','time':1600,'image':'http://www.sinmiedo.es/wp-content/uploads/2009/01/wii.jpg','countdown':False,'bidders':[],"current_winner":""}
        self.auctions["PS3"] = {'price': 600, 'name': 'PS3', 'desc':
            'Playstation 3 Giant Crab','time':50,'image':'http://www.hoytecnologia.com/img/noticias/foto_104367.jpg','countdown':False,'bidders':[],"current_winner":""}
        self.auctions["XBX"] = {'price': 490, 'name': 'Xbox', 'desc':
            'Microsoft Xbox 360','time':2200,'image':'http://cache.gizmodo.com/images/xbox360.jpg','countdown':False,'bidders':[],"current_winner":""}
