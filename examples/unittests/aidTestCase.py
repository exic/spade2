...
class AidTestCase(unittest.TestCase):
    ...
    def testEqual(self):
        a1 = spade.AID.aid(name="name1", addresses=["addr1",
            "addresses2"], resolvers=["resolver1"])
        a2 = spade.AID.aid()
        a2.setName("name1")
        a2.addAddress("addr1")
        ...
        a1 = spade.AID.aid(name="name1", addresses=["addr1"],
            resolvers=["resolver1"])
        self.assertNotEqual(a1, a2)
        a1 = spade.AID.aid(name="name1_1", addresses=["addr1",
            "addresses2"], resolvers=["resolver1"])
        self.assertNotEqual(a1, a2)
    ...
    def testMatch(self):
        a1 = spade.AID.aid(name="name1", addresses=["addr1",
            "addresses2"], resolvers=["resolver1"])
        a2 = spade.AID.aid(name="name1", addresses=["addr1"],
            resolvers=["resolver1"])

        self.assertEqual(a1.match(a2), True)

        a2 = spade.AID.aid(name="nam", addresses=["addr1",
            "addresses2"], resolvers=["resolver1"])
        self.assertEqual(a1.match(a2), True)
        ...
        a2 = spade.AID.aid(name="other", addresses=["addr1",
            "addresses2"], resolvers=["resolver1"])
        self.assertEqual(a1.match(a2), False)

        a2 = spade.AID.aid(name="name1", addresses=["addr2"],
            resolvers=["resolver1"])
        self.assertEqual(a1.match(a2), False)

    def testXML(self):
        xml1='<agent-identifier>\n\t<name>name1</name>\n\t<addresses>\n\t\t<url>addr1</url>\n\t\t<url>addresses2</url>\n\t</addresses>\n</agent-identifier>\n'
        aid1 = spade.AID.aid(name="name1", addresses=["addr1",
            "addresses2"], resolvers=["resolver1"])
        xml2 = aid1.asXML()
