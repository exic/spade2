            ...
            if self.msg.getType() == "subscribed":
                if self.msg.getFrom() == self.myAgent.getAMS().getName():
                    ...
                    self.DEBUG("Agent: " + str(self.
                        myAgent.getAID().getName()) + " registered correctly (inform)","ok")
                else:
                ...
                if self.msg.getFrom() == self.myAgent.getAMS().getName():
                    self.DEBUG(
                        "There was an error registering in the AMS: " + str(self.getAID().getName()),"err")
                else:
                    self.DEBUG(str(self.msg.getFrom())
                         + " has unsubscribed me")
        ...
        if stanza != None:
            for item in stanza.getTag('query').getTags('item'):
                ...
                if item.getAttr('subscription') == 'remove':
                    if self.myAgent._roster.has_key(
                        jid):
                ...
                self.myAgent._roster[jid]['subscription']
                    =item.getAttr('subscription')
                self.myAgent._roster[jid]['groups'] = []
                if not self.myAgent._roster[jid].has_key(
                    'resources'):
