    ...
    class P2PRequestHandler(SocketServer.StreamRequestHandler):
        def handle(self):
            ...
            try:
                ...
                self.server.owner.DEBUG(
                    "P2P Socket Closed to "+ str(self.client_address),"err")  #,":",str(e),":",str(length),str(data)
    ...
    def onStart(self):
        ...
        while open == False:
            try:
                ...
                self.myAgent.DEBUG(
                    "Changing P2P port to " + str(self.myAgent.P2PPORT))

        self.server._jabber_messageCB = self.myAgent._jabber_messageCB
        self.server.postMessage = self.myAgent.postMessage
        self.server.requests = []
        self.myAgent.DEBUG(self.getName() +
            ": P2P Behaviour Started at port " + str(self.myAgent.P2PPORT))
        ...
        if self.msg != None:
            if self.msg.getType() == "set":
                if self.msg.getTag("si").getAttr("profile") == "http://jabber.org/protocol/si/profile/spade-p2p-messaging":
                    #P2P Messaging Offer
                    self.myAgent.DEBUG(
                        "P2P-Messaging offer from " + str(self.msg.getFrom()))
                    if self.myAgent.p2p_ready:
                        ...
                        self.myAgent.DEBUG(
                            "P2P: Accept offer from "+str(self.msg.getFrom())+"."+str(reply) )
                    else:
                        #Refuse offer
                        reply = self.msg.buildReply("error")
                        err = reply.addChild(
                            "error", attrs={"code": "403", "type": "cancel"})
                        err.addChild("forbidden")
                        err.setNamespace("urn:ietf:params:xml:ns:xmpp-stanzas")
                        self.myAgent.DEBUG(
                            "P2P: Refuse offer from "+str(self.msg.getFrom())+"."+str(reply))
        ...
        if self.msg != None:
            if self.msg.getType() == "get":
                ...
                if self.myAgent.p2p_ready:
                    ...
                    reply.getTag("query").addChild(
                        "feature", {"var":"http://jabber.org/protocol/si/profile/spade-p2p-messaging"})
                self.myAgent.send(reply)
                self.myAgent.DEBUG(self.myAgent.getName() +
                    ": Sent Disco reply to " + str(reply.getTo()))
                ...
                if "http://jabber.org/protocol/si/profile/spade-p2p-messaging" not in services:
                    ...
                    if str(frm) in self.myAgent.p2p_routes.keys():
                        ...
                        try:
                            self.myAgent.DEBUG(
                                "Deleting "+str(frm)+" from P2P routes: "+ str(self.myAgent.p2p_routes),"warn")
            ...
            self.myAgent.DEBUG(
                "DiscoBehaviour returned with no message", "warn")
    ...
    def _process(self):
        ...
        if msg:
            ...
            if msg.getType() == "result":
                ...
                try:
                    remote_address = str(msg.getTag(
                        "si").getTag("p2p").getTag("value").getData())
                    d = {"url": remote_address, "p2p": True}
                    if self.myAgent.p2p_routes.has_key(str(msg.getFrom().getStripped())):
                        self.myAgent.p2p_routes[
                            str(msg.getFrom().getStripped())].update(d)
                        self.result = True
                    else:
                        self.myAgent.p2p_routes[
                            str(msg.getFrom().getStripped())] = d
                except Exception, e:
                    self.myAgent.DEBUG(
                        "Malformed StreamRequest Answer: " + str(e), "err")
                    self.myAgent.p2p_routes[str(msg.
                        getFrom().getStripped())] = {}
            elif msg.getType() == "error":
                self.myAgent.DEBUG("StreamRequest REFUSED", "warn")
                self.myAgent.p2p_routes[str(msg.getFrom().
                    getStripped())] = {'p2p': False}
        else:
...
class RequestDiscoInfoBehav(Behaviour.OneShotBehaviour):
    ...
    def __init__(self, to):
        ...
        self.temp_iq = xmpp.Iq(queryNS=xmpp.NS_DISCO_INFO, attrs={
            'id': self.id})
    ...
    def _process(self):
        ...
        if msg:
            if msg.getType() == "result":
                ...
                self.myAgent.DEBUG(
                    "Disco Info returned no results from " + str(self.to), "warn")
        else:
            self.myAgent.DEBUG(
                "No Disco Info retrieved from " + str(self.to), "warn")
