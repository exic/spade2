        ...
        def _process(self):
            ...
            if msg != None:
                    ...
                    if msg.getOntology().lower() == "fipa-agent-management":
                        if msg.getLanguage().lower() == "fipa-sl0":
                            ...
                            if co and co.has_key("fipa:action") and co["fipa:action"].has_key("fipa:act"):
                                ...
                                co2[
                                    "unsuported-function"] = co["fipa:action"]["fipa:act"]
                ...
                if error:
                    ...
                    self.myAgent.send(reply)
                    self.myAgent.DEBUG(
                        "NOT-UNDERSTOOD: Could not process message. Error is:"+str(error),'error')
                    return -1
        ...
        def _process(self):
            ...
            if "rdf" not in self.msg.getLanguage():
                ...
                try:
                    if self.content.action == "register":
                        dad = DfAgentDescription(
                            self.content.action.register['df-agent-description'])
                    else:
                        dad = DfAgentDescription(
                            self.content.action.deregister['df-agent-description'])
                except KeyError:  # Exception,err:
                    error = "(missing-argument df-agent-description)"
                    self.myAgent.DEBUG(
                        "REFUSE: Register Behaviour could not extract DfAgentDescription "+error,'error')

                if error:
                    reply = self.msg.createReply()
                    reply.setSender(self.myAgent.getAID())
                    reply.setPerformative("refuse")
                    reply.setContent("( " + self.msg.
                        getContent() + error + ")")
                    self.myAgent.send(reply)
                ...
                if self.content.action == "register":
                    ...
                    if not self.myAgent.servicedb.has_key(dad.getAID().getName()):
                        ...
                        try:
                            ...
                            reply.setPerformative("failure")
                            reply.setContent(
                                "("+self.msg.getContent() + "(internal-error))")
                            self.myAgent.send(reply)
                            self.myAgent.DEBUG(
                                "FAILURE: Service could not be registered: "+str(err),'error')
                            return -1

                    self.DEBUG(
                        "Service succesfully deregistered: " + str(dad), 'ok')
                    ...
                    if not self.myAgent.servicedb.has_key(dad.getAID().getName()):
                        reply.setPerformative("failure")
                        reply.setContent("(" + self.
                            msg.getContent() + "(not-registered))")
                        self.myAgent.send(reply)
                        self.myAgent.DEBUG(
                            "FAILURE: Agent has no registered services! Could not deregister "+str(dad),'warn')
                        return -1
                    ...
                    for ss in dad.getServices():
                        ...
                        if not found:
                            ...
                            self.myAgent.send(reply)
                            self.myAgent.DEBUG(
                                "FAILURE: Service is not registered! Could not deregister "+str(dad),'warn')
                            return -1

                    try:
                        services = copy.copy(self.
                            myAgent.servicedb[dad.getAID().getName()])
                        if dad.getServices() == []:
                            self.myAgent.db_mutex.acquire()
                        ...
                        reply.setPerformative("failure")
                        reply.setContent("(" + self.
                            msg.getContent() + '(internal-error "could not deregister service"))')
                        self.myAgent.send(reply)
                        self.myAgent.DEBUG(
                            "FAILURE: Service could not be deregistered: "+str(err),'error')
                        return -1
                ...
                try:
                    ...
                    self.myAgent.DEBUG(
                        "DfAgentDescription extracted "+str(dad.asRDFXML()),'info')
                except KeyboardInterrupt, err:
                    co_error = ContentObject(
                        namespaces={"http://www.fipa.org/schemas/fipa-rdf0#":"fipa:"})
                    co_error["fipa:error"]
                        = "missing-argument df-agent-description"
                    self.myAgent.DEBUG("REFUSE: " + str(
                        co_error) + ": " + str(err), 'error')
                ...
                if co["fipa:action"]["fipa:act"] == "register":
                    ...
                    self.DEBUG(
                        "Service succesfully registered: " + str(dad), 'ok')
                    reply.setPerformative("inform")
                    co_rep = ContentObject(namespaces={
                    ...
                    if not self.myAgent.servicedb.has_key(dad.getAID().getName()):
                        ...
                        self.myAgent.send(reply)
                        self.myAgent.DEBUG(
                            "FAILURE: Agent has no registered services", 'warn')
                        return -1
                    ...
                    try:
                        services = copy.copy(self.
                            myAgent.servicedb[dad.getAID().getName()])
                        self.myAgent.DEBUG(
                            "Deregistering "+str(services) + " AND " + str(dad.getServices()))
                        if dad.getServices() == []:
                            self.myAgent.db_mutex.acquire()
                            del self.myAgent.servicedb[dad.getAID().getName()]
                            self.DEBUG(
                                "Deleting all agent entries: " + str(not self.myAgent.servicedb.has_key(dad.getAID().getName())))
                            self.myAgent.db_mutex.release()
                        else:
                            for ss in dad.getServices():
                                for s in services.getServices():
                        ...
                        self.myAgent.send(reply)
                        self.myAgent.DEBUG(
                            "FAILURE: internal-error 'could not deregister service': "+str(err),'error')
                        return -1
            ...
            if not rdf:
                ...
                if "search-constraints" in self.content.action.search:
                    if "max-results" in self.content.action.search["search-constraints"]:
                        try:
                            max_str = str(self.
                                content.action.search["search-constraints"]["max-results"]).strip("[']")
                            max = int(max_str)
                        except Exception, err:
                            error = '(internal-error "max-results is not an integer")'
                            self.myAgent.DEBUG(
                                "FAILURE: internal-error 'max-results is not an integer' "+str(err),'error')
                if error:
                    reply = self.msg.createReply()
                    reply.setSender(self.myAgent.getAID())
                    reply.setPerformative("failure")
                ...
                if "df-agent-description" in self.content.action.search:
                    try:
                        dad = DfAgentDescription(
                            self.content.action.search["df-agent-description"])
                    except Exception, err:
                        self.myAgent.DEBUG(
                            "FAILURE: Could not extract DfAgentDescription from content: "+str(err),'error')
                ...
                if len(result) > 0:
                    ...
                    pass
                content += "))"
                self.myAgent.DEBUG("Found " + str(len(
                    result)) + " services", 'ok')
                for d in result:
                ...
                if self.content.action.argument.max_results:
                    try:
                        ...
                        self.myAgent.DEBUG(
                            "FAILURE: (internal-error) max-results is not an integer! ",'error')
                        #co_error = ContentObject()
                    ...
                    if self.content.action.argument.df_agent_description:
                        try:
                            ...
                            self.myAgent.DEBUG(
                                "Searching for: " +str(dad) + " in ServiceDB: " + str(map(lambda s: str(s), self.myAgent.servicedb.values())) )
                        except Exception, err:
                            self.myAgent.DEBUG(
                                "FAILURE: Could not extract DfAgentDescription from content: "+str(err),'error')
                    ...
                    content = ContentObject(namespaces=
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                {"http: //www.fipa.org/schemas/fipa-rdf0#": "fipa: "})
                    content["fipa:result"] = []
                    for i in result:
                        content["fipa:result"].append(i.asContentObject())
                        self.myAgent.DEBUG(str(i), 'ok')
                    self.myAgent.DEBUG("Found " + str(
                        len(result)) + " services.", 'ok')
                    reply.setPerformative("inform")
            ...
            if "rdf" in self.msg.getLanguage():
                ...
                if self.myAgent.servicedb.has_key(dad.getAID().getName()):
                    ...
                    try:
                        ...
                        self.myAgent.send(reply)
                        self.myAgent.DEBUG(
                            "FAILURE: internal-error: " + str(err), 'error')
                        return -1
                    ...
                    self.myAgent.send(reply)
                    self.myAgent.DEBUG(
                        "FAILURE: Could not modify service "+str(dad)+". Agent has no registered services.",'warn')
                    return -1

            else:
                #language is sl-0
                try:
                    dad = DF.DfAgentDescription(self.
                        content.action.modify[0][1])
                except Exception, err:
                ...
                if dad and (dad.getAID().getName() != self.msg.getSender().getName()):
                    error = "(unauthorised)"
                    self.myAgent.DEBUG("REFUSE: " + self.
                ...
                if self.myAgent.servicedb.has_key(dad.getAID().getName()):
                    ...
                    try:
                        ...
                        reply.setPerformative("failure")
                        reply.setContent("(" + self.
                            msg.getContent() + "(internal-error))")
                        self.myAgent.send(reply)
                        self.myAgent.DEBUG(
                            "FAILURE: (internal-error) Modifying service: "+str(err),'error')
                        return -1
        ...
        if content != None:
            ...
            if "properties" in content:
                for p in content.properties.set.asDict().values():
                    self.properties[str(p['name']).
                        lower().strip("[']")] = str(p['value']).lower().strip("[']")
...
class Service:
    ...
    def asContentObject(self):
        ...
        if self.getOwner() != None:
            co.service[
            "owner"] = self.getOwner().asContentObject()
        if self.getOntology() != []:
