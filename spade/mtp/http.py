...
FIPA_HR = {'Content-type': 'text/plain', 'Cache-Control': 'no-cache',
    'Connection': 'close'}

FIPA_HR_LOWER = {'content-type': 'text/plain', 'cache-control': 'no-cache',
    'connection': 'close'}
        ...
        for receiver in to:
            ...
            for ad in receiver.getAddresses():
                ...
                if ad[0:7] == "http://":
                    ...
                    conn.request("POST", remote_path,
                         body, self.fipaHeadersPost)
    ...
    def do_POST(self):
        ...
        for i in FIPA_HP.keys():
            if i.lower() not in new_headers.keys():
                print "HEADERS MISMATCH", str(self.headers.
                    keys()), str(FIPA_HP.keys())
