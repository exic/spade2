    ...
    def _process(self):
        ...
        if self.msg != None:
            if self.msg.getType() == "set":
                ...
                self.myAgent.DEBUG(
                    "RPC request received: " + str(self.msg), 'info', "rpc")

                mc = self.msg.getTag('query')
                params, name = xmlrpclib.loads(
                    "<?xml version='1.0'?>%s" % str(mc))
                name = name.lower()
                self.myAgent.DEBUG(
                    "Params processed: name="+name+" params="+str(params) + " in " + str(self.myAgent.RPC.keys()),"info","rpc")

                if not name in self.myAgent.RPC:
                    self.myAgent.DEBUG(
                        "RPC: 404 method not found", 'error', "rpc")
                    xmlrpc_res = xmlrpclib.dumps(
                         xmlrpclib.Fault(404, "method not found"))
                    reply = self.msg.buildReply("error")
                    reply.setQueryPayload([xmpp.
                        simplexml.XML2Node(xmlrpc_res)])
                ...
                self.myAgent.DEBUG("service and method: " +
                     str(service) + " --> " + str(methodCall), "info", "rpc")

                self.myAgent.DEBUG(
                    "Comparing service.getP(): "+ str(service.getP()) + " with params--> " + str(params),"info","rpc")
                ps = params[0].keys()
                for p in service.getP():
                    if str(p) not in ps:
                        self.myAgent.DEBUG(
                            "RPC: 500 missing precondition: "+str(p)+ " is not in "+str(params),'error',"rpc")
                        xmlrpc_res = xmlrpclib.dumps( xmlrpclib.Fault(500, "missing precondition"))
                        reply = self.msg.buildReply("error")
                        reply.setQueryPayload([
                            xmpp.simplexml.XML2Node(xmlrpc_res)])
                        reply.setType("result")
                        reply.setFrom(self.myAgent.JID)
                        self.myAgent.send(reply)
                        return
                ...
                try:
                    self.myAgent.DEBUG(
                        "Calling method "+str(methodCall)+" with params "+str(params),"info","rpc")
                    if params == (None,):
                        result = methodCall()
                    else:
                        args = ""
                    ...
                    self.myAgent.DEBUG(
                        "RPC: 500 method error: " + str(e), 'error', "rpc")
                    xmlrpc_res = xmlrpclib.dumps(
                ...
                try:
                    ...
                    if type(result) == types.DictType:
                        ...
                        self.myAgent.DEBUG(
                            "RPC method MUST return a dict.", 'error', "rpc")
                        fail = True
                except:
                    fail = True
                if fail:
                    self.myAgent.DEBUG(
                        "RPC: 500 missing postcondition: "+str(service.getQ()),'error',"rpc")
                    xmlrpc_res = xmlrpclib.dumps(
                ...
                self.myAgent.send(reply)
                self.myAgent.DEBUG(
                    "RPC: method succesfully served: "+ str(reply), 'ok', "rpc")

        else:
            self.myAgent.DEBUG(
                "RPCServerBehaviour returned with no message", "warn", "rpc")
    ...
    def _process(self):
        ...
        for p in ps:  # check all Preconditions are True
            if not p in self.inputs.keys():
                ...
                self.myAgent.DEBUG("Precondition " + str(p)
                     + " is not satisfied. Can't call method "+self.service.getName(),'error',"rpc")
                self.result = False
                return
            else:
                params[p] = self.inputs[p]
        ...
        payload = xmlrpclib.dumps((params,), methodname=self.
            service.getName(), allow_none=True)
        ...
        iq = xmpp.protocol.Iq(typ='set', queryNS="jabber:iq:rpc",
        ...
        self.myAgent.DEBUG(self.service.getName() +
             " method called. Waiting for response", 'ok', "rpc")
        ...
        if self.msg != None:
            ...
            if self.msg.getType() == "result":
                try:
                    params, method = xmlrpclib.loads(
                        "<?xml version='1.0'?>%s" % self.msg)
                    ...
                    self.myAgent.DEBUG(
                        "Error executing RPC service: " +str(e), "error", "rpc")
                    self.result = False
                    return False
            else:
                self.result = False
