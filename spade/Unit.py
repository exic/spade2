...
class Unit(Behaviour.OneShotBehaviour):
    ...
    def __init__(self,agent, nick, name, type="Team", goalList=[], agentList=[], contentLanguage="sl",password=None,create=True):
        ...
        self.parent = None
        id_base = "".join([string.ascii_letters[int(random.randint(
            0,len(string.ascii_letters)-1))] for a in range(5)]) #@UnusedVariable
    ...
    def myCreate(self):
        ...
        #anyadimos el comportamiento que lee los presence
            ...
            self.presenceBehaviour = self.PresenceBehaviour(self.
                muc_name, self.name, self.nick, self)
    ...
    def testRoomName(self):
        ID = "".join([string.ascii_letters[int(random.randint(0, len(
            string.ascii_letters) - 1))] for a in range(5)])
    ...
    def createRoom(self):
        ...
        b = self.CreateRoomBehaviour(ID, self.muc_name, self.name, self.
            nick,self.parent,self.contentLanguage,self.type,self.goalList,self.password,self.agentList)
        self.myAgent.addBehaviour(b, t1)
        b.join()
        return b.result

    class CreateRoomBehaviour(Behaviour.OneShotBehaviour):
        def __init__(self,ID,muc_name,roomname,nick,parent,contentLanguage,type,goalList,password,agentList):
            Behaviour.OneShotBehaviour.__init__(self)
            self.result = False
            self.ID = ID
            self.nick = nick
        ...
        def _process(self):
            ...
            self.setTemplate(t)
            iq = Iq(to=self.name + "@" + self.muc_name, typ='get',
                 attrs={"id": self.ID})
            query = Protocol('query', xmlns=
        ...
        #setting room configuration
            ...
            iq = Iq(to=self.name + "@" + self.muc_name, typ='set',
                 attrs={"id": self.ID})
            query = Protocol('query', xmlns=
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                "http://jabber.org/protocol/muc#owner")
            x = Node(tag="x", attrs={"xmlns": "jabber: x: data",
                " type": "submit"})
            resquery = msg.getQueryChildren()[0]
                 #nos quedamos con el hijo de query
            if resquery:
                ...
                if item.getAttr("var"):
                    ...
                    if item.getAttr("var") == "muc#roomconfig_membersonly":
                        if self.type == "Flat":
                            ...
                            value = "1"
                    node = Node(tag="field", attrs={
                        "var": item.getAttr("var")})
            ...
            if msg and msg.getAttr("type")=="result": #comprobar mejor el mensaje que se devuelve
                ...
                m = Message(to=self.name + "@" + self.
                    muc_name, typ="groupchat")
                ...
                for agent in self.agentList:
                    iq = Iq(to=self.name + "@" + self.
                        muc_name, typ='set', attrs={"id": self.ID})
                    query = Protocol('query', xmlns=
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                "http://jabber.org/protocol/muc#admin")
                    item = Node(tag="item", attrs={
                        "affiliation": "member", "jid": agent})
    ...
    def myJoinRoom(self):
        p = Presence(frm=self.name + "@" + self.muc_name, attrs={
            "type": "error"})
        t1 = Behaviour.MessageTemplate(p)
        b = self.MyJoinRoomBehaviour(self.muc_name, self.name, self.
            nick, self.password)
        self.myAgent.addBehaviour(b, t1)
        b.join()
        return b.result
    ...
    def invite(self, agentList):
        ...
        for agent in agentList:
            message = Node(tag="message", attrs={"to": self.
                name + "@" + self.muc_name})
            x = Node(tag="x", attrs={"xmlns":
    ...
    def getGoal(self):
        ...
        ID = "".join([string.ascii_letters[int(random.randint(0, len(
            string.ascii_letters) - 1))] for a in range(5)])
        p = Iq(frm=self.name + "@" + self.muc_name, typ='result',
             attrs={"id": ID})
        t1 = Behaviour.MessageTemplate(p)
                            ...
                            if item.getAttr("var") == "muc#roominfo_type":
                                if item.getTags("value")[0].getData():
                                    if ':' in item.getTags("value")[0].getData():
                                        info["parent"]= item.getTags("value")[0].getData().split(':')[1]
                                    else:
    ...
    class AddAdminBehaviour(Behaviour.OneShotBehaviour):
        ...
        def _process(self):
            ...
            query = Protocol('query', xmlns=
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                "http://jabber.org/protocol/muc#admin")
            item = Node(tag="item", attrs={"affiliation":
                "admin", "jid": self.newAdminJID})
            query.addChild(node=item)
            iq.addChild(node=query)
            self.myAgent.jabber.send(iq)
            msg = self._receive(True, 10)
    ...
    class RemoveAdminBehaviour(Behaviour.OneShotBehaviour):
        ...
        def _process(self):
            ...
            if msg:
                ...
                query = msg.getQueryChildren()
                if query:
                    iqAns = Iq(to=self.name + "@" + self.
                        muc_name, typ='set', attrs={"id": self.ID})
                    queryAns = Protocol('query', xmlns=
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                "http://jabber.org/protocol/muc#admin")
                    for item in query:
                        if str(item.getAttr("jid")) == self.newAdminJID:
                            i = Node(tag="item",
                                attrs={"affiliation":"member","jid":self.newAdminJID})
    ...
    def addOwner(self, newOwnerJID):
        ...
        p = Iq(frm=self.name + "@" + self.muc_name, attrs={"id": ID})
        t = Behaviour.MessageTemplate(p)
        b = self.AddOwnerBehaviour(self.muc_name, self.name, ID, newOwnerJID)
        self.myAgent.addBehaviour(b, t)
        b.join()

    class AddOwnerBehaviour(Behaviour.OneShotBehaviour):
        def __init__(self, muc_name, roomname, ID, newOwnerJID):
            Behaviour.OneShotBehaviour.__init__(self)
            self.ID = ID
            self.muc_name = muc_name
            self.name = roomname
            self.newOwnerJID = newOwnerJID
            self.result = None
    ...
    def removeOwner(self, OwnerJID):
        ...
        p = Iq(frm=self.name + "@" + self.muc_name, attrs={"id": ID})
        t = Behaviour.MessageTemplate(p)
        b = self.RemoveOwnerBehaviour(self.muc_name, self.name, ID, OwnerJID)
        self.myAgent.addBehaviour(b, t)
        b.join()

    class RemoveOwnerBehaviour(Behaviour.OneShotBehaviour):
        def __init__(self, muc_name, roomname, ID, OwnerJID):
            Behaviour.OneShotBehaviour.__init__(self)
            self.ID = ID
            self.muc_name = muc_name
            self.name = roomname
            ...
            if msg:
                ...
                if query:
                    ...
                    queryAns = Protocol('query', xmlns=
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                "http://jabber.org/protocol/muc#admin")
                    for item in query:
                        if str(item.getAttr("jid")) == self.OwnerJID:
                            i = Node(tag="item",
                                attrs={"affiliation":"member","jid":self.OwnerJID})
                    ...
                    if msgAns:
                        if msgAns.getAttr("type") != "result":
                            error = msgAns.getTag("error")
                            if error.getAttr("code") == "409":
                                raise LastOwner
                        else:
                            exists = True
                if not exists:
                    print "The JID " +self.OwnerJID+" doesn't belong to a owner"
                return
            print "Error"#completar un poco el error "No se pudede quitar al unico owner"
    ...
    def addModerator(self, newModeratorJID):
        ...
        p = Iq(frm=self.name + "@" + self.muc_name, attrs={"id": ID})
        t = Behaviour.MessageTemplate(p)
        b = self.AddModeratorBehaviour(self.muc_name, self.name, ID,
            newModeratorJID)
        self.myAgent.addBehaviour(b, t)
        b.join()
    ...
    def removeModerator(self, moderatorJID):
        ...
        p = Iq(frm=self.name + "@" + self.muc_name, attrs={"id": ID})
        t = Behaviour.MessageTemplate(p)
        b = self.RemoveModeratorBehaviour(self.muc_name, self.name, ID,
            moderatorJID)
        self.myAgent.addBehaviour(b, t)
        b.join()
                ...
                if query:
                    ...
                    for item in query:
                        if item.getAttr("jid") == self.moderatorJID:
                            i = Node(tag="item",
                                attrs={"affiliation":"member","jid":self.moderatorJID})
                            queryAns.addChild(node=i)
                        else:
                            exists = True
                    self.myAgent.register_mailbox(typ="iq", id=self.ID)
    ...
    def leave(self):
        ...
        if self.myAgent.JID not in owners or len(owners) > 1:
            p = Presence(to=self.name + "@" + self.muc_name + "/" +
                self.nick, typ="unavailable")
            self.myAgent.jabber.send(p)
            self.state = "unavailable"
            ...
            if msg:
                if msg.getType() == "unavailable":
                    if msg.getRole()=="none" and msg.getFrom()==self.name+"@"+self.muc_name+"/"+self.nick:
                        ...
                        ID = "".join([string.
                            ascii_letters[int(random.randint(0,len(string.ascii_letters)-1))] for a in range(5)])
                        p = Iq(frm=self.name + "@" +
                            self.muc_name, attrs={"id": ID})
                        t1 = Behaviour.MessageTemplate(p)
                        b = self.MinAgentsBehaviour(
                            self.muc_name, self.name, ID, self.unit)
                        self.myAgent.addBehaviour(b, t1)
                else:
                    if (msg.getAffiliation()=="admin" or msg.getAffiliation()=="owner") and msg.getFrom()==self.name+"@"+self.muc_name+"/"+self.nick:
                        self.unit.owner_admin = True
                    if msg.getAffiliation()=="member" and msg.getFrom()==self.name+"@"+self.muc_name+"/"+self.nick:
                        self.unit.owner_admin = False
                    if self.unit.owner_admin:
                        if msg.getFrom() not in self.unit.members:
                            self.unit.members.append(msg.getFrom())
                            ID = "".join([string.
                                ascii_letters[int(random.randint(0,len(string.ascii_letters)-1))] for a in range(5)])
                            p = Iq(frm=self.
                                name + "@" + self.muc_name, attrs={"id": ID})
                            t1 = Behaviour.MessageTemplate(p)
        ...
        class MinAgentsBehaviour(Behaviour.OneShotBehaviour):
            ...
            def _process(self):
                ...
                if msg:
                    ...
                    agents = None
                    iq = Iq(to=self.name + "@" + self.
                        muc_name, typ='get', attrs={"id": self.ID})
                    query = Protocol('query', xmlns=
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                "http://jabber.org/protocol/disco#info")
                    iq.addChild(node=query)
                    self.myAgent.jabber.send(iq)
                    ...
                    if agents and minAgents and int(agents) < int(minAgents):
                        iq = Iq(to=self.name + "@" +
                            self.muc_name, typ='get', attrs={"id": self.ID})
                ...
                if agents and minAgents and int(agents)>int(minAgents) and self.unit.state=="locked":
                    ...
                    if msg:
                        ...
                        if q:
                            iq = Iq(to=self.
                                name+"@"+self.muc_name,typ='set', attrs={"id":self.ID})
                            query = Protocol(
                                'query',xmlns="http://jabber.org/protocol/muc#admin")
                            items = q.getChildren()
                            for item in items:
                                i = Node(
                                    tag="item", attrs={"jid":str(item.getAttr("jid")),"role":"visitor"})
                                query.addChild(node=i)
                            iq.addChild(node=query)
                            self.myAgent.jabber.send(iq)
    ...
    class KickAgentBehaviour(Behaviour.OneShotBehaviour):
        ...
        def _process(self):
            ...
            query = Protocol('query', xmlns=
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                "http://jabber.org/protocol/muc#admin")
            item = Node(tag="item", attrs={"role": "none",
                "nick": self.agentNick})
            query.addChild(node=item)
            iq.addChild(node=query)
            self.myAgent.jabber.send(iq)
    ...
    def removeBanAgent(self, agentJID):
        ...
        p = Iq(frm=self.name + "@" + self.muc_name, attrs={"id": ID})
        t = Behaviour.MessageTemplate(p)
        b = self.RemoveBanAgentBehaviour(self.muc_name, self.name,
             ID, agentJID)
        self.myAgent.addBehaviour(b, t)
        b.join()
        return b.result
    ...
    class GiveVoiceBehaviour(Behaviour.OneShotBehaviour):
        ...
        def _process(self):
            ...
            query = Protocol('query', xmlns=
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                "http://jabber.org/protocol/muc#admin")
            item = Node(tag="item", attrs={"nick": self.nickname,
                "role": "participant"})
            query.addChild(node=item)
            iq.addChild(node=query)
            self.myAgent.jabber.send(iq)
    ...
    def addUnit(self, unit):
        ...
        #un sitwch para aquellas organizaciones donde todos puedan crear unidades
            if self.parent_type != "Matrix" and self.parent_type !="Federation":
                if self.myAgent.JID in self.getAdminList():
                    unit.create = True
    ...
    def getSupervisorList(self):
        ...
        ID = "".join([string.ascii_letters[int(random.randint(0, len(
            string.ascii_letters) - 1))] for a in range(5)])
        p = Iq(frm="Team: " + self.parent + "@" + self.muc_name,
             attrs={"id": ID})
        t1 = Behaviour.MessageTemplate(p)
        b = self.GetMemberListBehaviour(self.muc_name, "Team:" +
            self.parent, ID)
        self.myAgent.addBehaviour(b, t1)
        b.join()
        member = b.result
        ...
        p = Iq(frm="Team: " + self.parent + "@" + self.muc_name,
             attrs={"id": ID})
        t = Behaviour.MessageTemplate(p)
        b = self.GetOwnerListBehaviour(self.muc_name, "Team:" +
             self.parent, ID)
        self.myAgent.addBehaviour(b, t)
        b.join()
        owner = b.result
        ...
        p = Iq(frm="Team: " + self.parent + "@" + self.muc_name,
             attrs={"id": ID})
        t = Behaviour.MessageTemplate(p)
        b = self.GetAdminListBehaviour(self.muc_name, "Team:" +
             self.parent, ID)
        self.myAgent.addBehaviour(b, t)
        b.join()
        admin = b.result
