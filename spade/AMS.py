...
#                       self.registerPresenceHandler(t, self.subscribeCB)
        ...
        class SubscribeBehaviour(Behaviour.Behaviour):
            def _process(self):
                ...
                if msg:
                    self.myAgent.DEBUG(
                        "AMS received presence message " + str(msg), "info")
                    typ = msg.getType()
                    frm = msg.getFrom()
                    ...
                    if typ == "subscribe":
                        ...
                        self.myAgent.DEBUG(
                            "AMS succesfully registered agent " + frm.getName(),"ok")
                        presence = xmpp.Presence(
                            reply_address, typ="subscribed")
                        presence.setFrom(self.myAgent.JID)
                        self.myAgent.DEBUG(
                            "AMS sends " + str(presence), "info")
                        self.myAgent.send(presence)
                    elif typ == "unsubscribe":
                        if self.myAgent.agentdb.has_key(str(frm)):
                            del self.myAgent.agentdb[str(frm)]
                            self.myAgent.DEBUG(
                                "Agent " + str(frm) + " deregistered from AMS","ok")
                        else:
                            self.myAgent.DEBUG(
                                "Agent " + str(frm) + " deregistered from AMS","error")
                return
        ...
        def _process(self):
            ...
            try:
                if "register" in self.content.action:
                    aad = AmsAgentDescription(self.
                        content.action.register['ams-agent-description'])
                else:
                    aad = AmsAgentDescription(self.
                        content.action.deregister['ams-agent-description'])
            ...
            if "register" in self.content.action:
                ...
                if self.myAgent.agentdb.has_key(aad.getAID().getName()):
                    try:
                        ...
                        reply.setPerformative("failure")
                        reply.setContent("(" + self.
                            msg.getContent() + '(internal-error "could not deregister agent"))')
                        self.myAgent.send(reply)
                        return -1
    ...
    class PlatformBehaviour(Behaviour.OneShotBehaviour):
        ...
        def _process(self):
            ...
            if rdf:
                ...
                co_serv["ap-service"]["name"]
                    = "xmpp://" + str(self.myAgent.getAMS().getName())
                co_serv["ap-service"]["type"] = "fipa.agent-management.ams"
                co_serv["ap-service"]["addresses"] = []
                co_serv["ap-service"]["addresses"].append(
                ...
                content += " :addresses (sequence " + str(
                    self.myAgent.getSpadePlatformJID()) + ")"
                content + " ) )"
            ...
            if not rdf:
                if "search-constraints" in self.content.action.search:
                    if "max-results" in self.content.action.search["search-constraints"]:
                        try:
                            max = int(self.
                                content.action.search["search-constraints"]["max-results"])
                        except Exception, err:
                ...
                if "ams-agent-description" in self.content.action.search:
                    aad = AmsAgentDescription(self.
                        content.action.search['ams-agent-description'])
                ...
                if "constraints" in co["fipa:action"]:
                    try:
                ...
                if co["fipa:action"].has_key("fipa:argument") and co["fipa:action"]["fipa:argument"]:
                    aad = AmsAgentDescription(co=co[
                        "fipa:action"]["fipa:argument"])
                    for a in self.myAgent.agentdb.values():
                        if max >= 0:
                            if a.match(aad):
                                result.append(a)
    ...
    class ModifyBehaviour(Behaviour.OneShotBehaviour):
        ...
        def _process(self):
            ...
            if not rdf:
                ...
                try:
                    aad = AmsAgentDescription(self.
                        content.action.modify['ams-agent-description'])
                except Exception, err:
                    error = "(missing-argument ams-agent-description)"
                    self.myAgent.DEBUG(
                        "Modify: Missing argument in ams-agent-description",'error')
                ...
                if aad.getAID() and aad.getAID().getName() == None:
                    ...
                    self.myAgent.DEBUG(
                        "Modify: Overwriting missing AID with sender AID "+ str(self.msg.getSender()),'warn')

                if aad and (not aad.getAID() == self.msg.getSender()):
                    error = "(unauthorised)"
                    self.myAgent.DEBUG(
                        "Modify: Unauthorised. AID does not match with sender",'error')
