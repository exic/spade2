...
class Organization(Unit.Unit):
    ...
    def createRoom(self):
        ...
        b = self.CreateRoomBehaviour(ID, self.muc_name, self.name, self.
            nick, self.contentLanguage, self.type, self.goalList)
    ...
    def createTeam(self):
        ID = "".join([string.ascii_letters[int(random.randint(0, len(
            string.ascii_letters) - 1))] for a in range(5)])
        p = Presence(frm="Team:" + self.name + "@" + self.muc_name +
            "/" + self.nick)
        t1 = Behaviour.MessageTemplate(p)
        b = self.CreateTeamBehaviour(ID, self.muc_name, self.name, self.
            nick, self.contentLanguage, self.goalList, self.agentList)
        self.myAgent.addBehaviour(b, t1)
        b.join()
        return b.result
    ...
    def checkType(self):
        types = ("Flat", "Team", "Hierarchy", "Bureaucracy", "Matrix",
            "Federation", "Coalition", "Congregation")
    ...
    class GetRegistrationFormBehaviour(Behaviour.OneShotBehaviour):
        ...
        def _process(self):
            iq = Iq(to=self.unitName + "@" + self.muc_name,
...
#cambiar
    ...
    def sendRegistrationForm(self, unitName, dataForm):
        ...
        p = Iq(frm=unitName + "@" + self.muc_name, attrs={"id": ID})
        t1 = Behaviour.MessageTemplate(p)
        b = self.GetRegistrationFormBehaviour(self.muc_name, self.
            name, ID, unitName, dataForm)
        self.myAgent.addBehaviour(b, t1)
        b.join()
        return b.result
            ...
            if msg:
                if query:
                    ...
                    for item in items:
                        if item.getAttr("jid"):
                            iq = Iq(to=item.
                                getAttr("jid"), typ='get', attrs={"id":self.ID})
                            query = Protocol(
    ...
    def destroy(self):
       ...
       Organization owner destroys the organization
        ...
        if self.state == "unavailable":
            ...
            return
        if self.getUnitList == []:
            ID = "".join([string.ascii_letters[int(random.
                randint(0, len(string.ascii_letters) - 1))] for a in range(5)])
    ...
    class IsMemberBehaviour(Behaviour.OneShotBehaviour):
        ...
        def _process(self):
            ...
            query = Protocol('query', xmlns=
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                "http://jabber.org/protocol/disco#items",attrs={"node":"http://jabber.org/protocol/muc#rooms"})
            iq.addChild(node=query)
            self.myAgent.jabber.send(iq)
            msg = self._receive(True, 10)
            if msg:
    ...
    def addUnit(self, unit):
        ...
        #un sitwch para aquellas organizaciones donde todos puedan crear unidades
            if self.type != "Matrix" and self.type != "Federation":
                if self.checkOwner(self.myAgent.JID):
                    unit.create = True
    ...
    def getSupervisorList(self):
        ...
        ID = "".join([string.ascii_letters[int(random.randint(0, len(
            string.ascii_letters) - 1))] for a in range(5)])
        p = Iq(frm="Team: " + self.name + "@" + self.muc_name,
             attrs={"id": ID})
        t1 = Behaviour.MessageTemplate(p)
