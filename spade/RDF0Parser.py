...
class RDF0Parser(handler.ContentHandler):
    ...
    def encodeTag(self, tag, content):
        ...
        if isinstance(content, dict):
            ...
            sb = self.encodeOneLineTag(tag, btag(self.
                RDF_PREF, self.RESOURCE), content[self.RESOURCE])
        else:
            if self.TYPE_SEP in content:
                #encoding value with type
                value, type = content.split(self.TYPE_SEP)
                sb = self.encodeInitTagAttr(tag, btag(
                                        self.RDF_PREF, self.DATATYPE), type)
                sb = sb + value.strip('"')
    ...
    def encode(self, content):
        ...
        if self.ACTION in content:
            if self.ID in content[self.ACTION]:
                sb_action = self.encodeInitTagAttr(btag(
                    self.FIPA_PREF,self.ACTION),btag(self.RDF_PREF,self.ID),content[self.ACTION][self.ID])

            elif self.ABOUT in content[self.ACTION]:
                sb_action = self.encodeInitTagAttr(btag(
                    self.FIPA_PREF,self.ACTION),btag(self.RDF_PREF,self.ABOUT),content[self.ACTION][self.ABOUT])

            else:
                sb_action = self.encodeInitTag(btag(self.
                    FIPA_PREF, self.ACTION))

            if self.ACTOR in content[self.ACTION]:
                sb_action = sb_action + self.encodeTag(
                    btag(self.FIPA_PREF,self.ACTOR),content[self.ACTION][self.ACTOR])

            if self.ACT in content[self.ACTION]:
                sb_action = sb_action + self.encodeTag(
                    btag(self.FIPA_PREF,self.ACT),content[self.ACTION][self.ACT])
            ...
            if self.ARGUMENT in content[self.ACTION]:
                ...
                if container == None:
                    ...
                    accum = self.encodeTag(btag(self.
                        RDF_PREF, container), accum)

                sb_action = sb_action + self.encodeTag(
                    btag(self.FIPA_PREF, self.ARGUMENT), accum)
            ...
            if self.IMPLBY in content[self.ACTION]:
                ...
                keys = [self.LANGUAGE, self.BINDING, self.
                    CODE_URI, self.SCRIPT]
                ...
                if container == None:
                    if self.CODE in content[self.ACTION][self.IMPLBY]:
                        ...
                        accum = self.encodeTag(
                            btag(self.FIPA_PREF, self.CODE), accum)
                ...
                sb_action = sb_action + self.encodeTag(
                    btag(self.FIPA_PREF, self.IMPLBY), accum)

            if self.DONE in content[self.ACTION]:
                sb_action = sb_action + self.encodeTag(
                    btag(self.FIPA_PREF,self.DONE),content[self.ACTION][self.DONE])

            if self.RESULT in content[self.ACTION]:
                sb_action = sb_action + self.encodeTag(
                    btag(self.FIPA_PREF,self.RESULT),content[self.ACTION][self.RESULT])

            sb = sb + sb_action + self.encodeEndTag(btag(self.
                FIPA_PREF, self.ACTION))
        ...
        if self.PROPOSITION in content:
            ...
            if self.SUBJECT in content[self.PROPOSITION]:
                accum = accum + self.encodeTag(btag(self.
                    RDF_PREF,self.SUBJECT),content[self.PROPOSITION][self.SUBJECT])
            if self.PREDICATE in content[self.PROPOSITION]:
                accum = accum + self.encodeTag(btag(self.
                    RDF_PREF,self.PREDICATE),content[self.PROPOSITION][self.PREDICATE])
            if self.OBJECT in content[self.PROPOSITION]:
                accum = accum + self.encodeTag(btag(self.
                    RDF_PREF,self.OBJECT),content[self.PROPOSITION][self.OBJECT])
            if self.BELIEF in content[self.PROPOSITION]:
                accum = accum + self.encodeTag(btag(self.
                    FIPA_PREF,self.BELIEF),content[self.PROPOSITION][self.BELIEF])

            sb = sb + self.encodeTag(btag(self.FIPA_PREF, self.
                PROPOSITION), accum)
        ...
        if self.DESCRIPTION in content:
            sb = sb + self.encodeInitTagAttr(btag(self.
                RDF_PREF,self.DESCRIPTION),btag(self.RDF_PREF,self.ABOUT),content[self.DESCRIPTION][self.ABOUT])
            if self.DONE in content[self.DESCRIPTION]:
                sb = sb + self.encodeTag(btag(self.
                    FIPA_PREF, self.DONE), content[self.DESCRIPTION][self.DONE])

            if self.RESULT in content[self.DESCRIPTION]:
                sb = sb_action + self.encodeTag(btag(self.
                    FIPA_PREF,self.RESULT),content[self.DESCRIPTION][self.RESULT])
    ...
    def startElementNS(self, name, qname, attributes):
        ...
        if str(self.RDF_NS+self.RESOURCE).lower() == str(name[0]+name[1]).lower():
            ...
            self.resource[name] = attributes.getValueByQName(
                btag(self.prefixs.rdf, self.RESOURCE))
        ...
        if str(self.RDF_NS+self.DATATYPE).lower() == str(name[0]+name[1]).lower():
            ...
            self.datatype[name] = attributes.getValueByQName(
                btag(self.prefixs.rdf, self.DATATYPE))
    ...
    def endElementNS(self, name, qname):
        ...
        if self.ACTOR.lower() == name[1].lower():
            ...
            if self.container != None:
                self.content[self.s][self.p][self.
                    container].append(self.accumulator)
            else:
                pass
            ...
            if self.container != None:
                self.content[self.s][self.p][self.
                    container].append({self.CODE: self.code})
            else:
