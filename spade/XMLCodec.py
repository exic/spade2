...
class XMLCodec(handler.ContentHandler):
    ...
    def encodeProp( self, prop):
            ...
            sb.append(PROP_ATTR).append("=\"").append(p.
                getName()).append("\" ");
...
"""
    ...
    def encodeXML(self, env):
        ...
        if env.getPayloadEncoding() != None and len(env.getPayloadEncoding()) > 0:
            sb = sb + self.encodeTag(self.ENCODING_TAG, env.
                getPayloadEncoding())
        ...
        if ro != None:
            ...
            if value != None:
                sb = sb + self.encodeOneLineTag(self.
                    RECEIVED_DATE, self.RECEIVED_ATTR, str(value))
            #By
            value = ro.getBy()
            if value != None:
                sb = sb + self.encodeOneLineTag(self.
                    RECEIVED_BY, self.RECEIVED_ATTR, value)
            #From
            value = ro.getFrom()
            if value != None:
                sb = sb + self.encodeOneLineTag(self.
                    RECEIVED_FROM, self.RECEIVED_ATTR, value)
            #Id
            value = ro.getId()
            if value != None:
                sb = sb + self.encodeOneLineTag(self.
                    RECEIVED_ID, self.RECEIVED_ATTR, value)
            #Via
            value = ro.getVia()
            if value != None:
                sb = sb + self.encodeOneLineTag(self.
                    RECEIVED_VIA, self.RECEIVED_ATTR, value)
    ...
    def startElement(self, localName, attributes):
        ...
        if self.TO_TAG.lower() == localName.lower():
            ...
            self.env.received.setFrom(attributes.getValue(
                                self.RECEIVED_ATTR))

        elif self.RECEIVED_DATE.lower() == localName.lower():
            self.env.received.setDate(BasicFipaDateTime(str(
                                attributes.getValue(self.RECEIVED_ATTR))))
