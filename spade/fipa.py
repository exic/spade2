    ...
    def _process(self):
        ...
        if msg == None or str(msg.getPerformative()) != 'agree':
            try:
                ...
                aadname = "<unknown>"
            self.myAgent.DEBUG(
                "There was an error searching the Agent " + aadname + "(not agree)","warn")
            self.finished = True
            return None
        ...
        if msg == None or msg.getPerformative() != 'inform':
            try:
                ...
                aadname = "<unknown>"
            self.myAgent.DEBUG(
                "There was an error searching the Agent " + aadname + "(not inform)","warn")
            self.finished = True
            return None
        else:
            try:
...
class ModifyAgentBehaviour(Behaviour.OneShotBehaviour):
    ...
    def _process(self):
        ...
        if msg == None or msg.getPerformative() != 'agree':
            self.myAgent.DEBUG(
                "There was an error modifying the requested Agent (not agree)","warn")
            self.result = False
            return False
        msg = self._receive(True, 20)
        if msg == None or msg.getPerformative() != 'inform':
            self.myAgent.DEBUG(
                "There was an error modifying the requested Agent (not inform)","warn")
...
class getPlatformInfoBehaviour(Behaviour.OneShotBehaviour):
    ...
    def _process(self):
        ...
        if msg == None or msg.getPerformative() != 'agree':
            self.myAgent.DEBUG(
                "There was an error getting platform info", "warn")
            return False
        msg = self._receive(True, 20)
        if msg == None or msg.getPerformative() != 'inform':
            self.myAgent.DEBUG(
...
class registerServiceBehaviour(Behaviour.OneShotBehaviour):
    ...
    def _process(self):
        ...
        if msg == None or msg.getPerformative() not in ['agree', 'inform']:
            self.myAgent.DEBUG(
                "There was an error registering the service " +str(self.DAD)+ "(not agree)","warn")
            self.result = False
            return False
            ...
            if msg == None or msg.getPerformative() != 'inform':
                if not msg:
                    self.DEBUG(
                        "There was an error registering the Service " +str(self.DAD)+ ". (timeout)","warn")
                elif msg.getPerformative() == 'failure':
                    self.DEBUG(
                        "There was an error registering the Service " +str(self.DAD)+ ". Failure: " +  msg.getContentObject()['fipa:error'],"warn")
                else:
                    self.DEBUG(
                        "There was an error registering the Service " +str(self.DAD)+ ". (not inform)","warn")
                self.result = False
                return False
...
class deregisterServiceBehaviour(Behaviour.OneShotBehaviour):
    ...
    def _process(self):
        ...
        if msg == None or msg.getPerformative() not in ['agree', 'inform']:
            self.myAgent.DEBUG(
                "There was an error deregistering the Service "+str(self.DAD) +". (not-agree)","warn")
            self.result = False
            return
        elif msg == None or msg.getPerformative() == 'agree':
            msg = self._receive(True, 20)
            if msg == None or msg.getPerformative() != 'inform':
                if not msg:
                    self.DEBUG(
                        "There was an error deregistering the Service "+str(self.DAD)+". (timeout)","warn")
                elif msg.getPerformative() == 'failure':
                    self.DEBUG(
                        "There was an error deregistering the Service "+str(self.DAD)+". failure:" +  msg.getContentObject()['fipa:error'],"warn")
                else:
                    self.DEBUG(
                        "There was an error deregistering the Service "+str(self.DAD)+". (not inform)","warn")
                self.result = False
                return
...
class searchServiceBehaviour(Behaviour.OneShotBehaviour):
    ...
    def _process(self):
        try:
            ...
            if msg == None:
                self.DEBUG(
                    "There was an error searching the Service (timeout on agree)","warn")
                return
            elif msg.getPerformative() not in ['agree', 'inform']:
                self.DEBUG(
                    "There was an error searching the Service (not agree) Failure: "+str(msg.getContentObject()["fipa:error"]),"warn")
                return
            elif msg.getPerformative() == 'agree':
                msg = self._receive(True, 10)
                if msg == None:
                    self.DEBUG(
                        "There was an error searching the Service (timeout on inform)","warn")
                    return
                elif msg.getPerformative() != 'inform':
                    self.DEBUG(
                        "There was an error searching the Service (not inform) " +  str(msg.getContentObject()['fipa:error']),"warn")
                    return
...
class modifyServiceBehaviour(Behaviour.OneShotBehaviour):
    ...
    def _process(self):
        ...
        if msg == None or msg.getPerformative() != 'agree':
            self.DEBUG(
                "There was an error modifying the Service. (not agree) ","warn")
            if msg:
        ...
        if msg == None or msg.getPerformative() != 'inform':
            self.DEBUG(
                 "There was an error modifying the Service. (not inform) " +  str(msg.getContentObject()['fipa:error']),"warn")
            self.result = False
            return
        self.result = True
        return
