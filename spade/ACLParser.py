...
class ACLParser:
    ...
    def __init__(self):
        ...
        Float = Combine((Optional(sign) + FloatMantissa + Optional(
            FloatExponent) ) | ( Optional(sign) + OneOrMore(digit) + FloatExponent ))
        ...
        login = (Optional(user + Optional(Literal(":")+password) +
             Literal("@")) + hostport )
        mailtoaddress = Combine(Literal("mailto:")+ xalphas +
             Literal("@") + hostname)
        ftpaddress = Combine(Literal("ftp://")+login+Literal("/")+
            path+Optional(Literal(";")+ftptype))
        httpaddress = Combine(Literal("http://")+hostport+Optional(
            Literal("/")+path) +Optional(Literal("?")+search))
        ...
        AgentIdentifierSequence = Group(lpar + Literal("sequence").
            suppress() + OneOrMore(AgentIdentifier) + rpar)#.setResultsName("AIDseq")
        ...
        Expression = Forward()
        Expression << (ACLWord | String | Number | DateTime | (
        ...
        Message = (lpar + MessageType.setResultsName("msgtype") +
             OneOrMore(MessageParameter.setResultsName("msgParameter")) + rpar)#.setResultsName("message")
    ...
    ***************************************************
    ...
    def startElement(self, localName, attributes):
        ...
        if self.FIPA_MESSAGE_TAG == localName.lower():
            ...
            try:
                self.msg.setConversationId(attributes.
                    getValue(self.CONVERSATION_ID_TAG))
        ...
        if self.REPLY_BY_TAG == localName.lower():
            self.msg.setReplyBy(BasicFipaDateTime(attributes.
                getValue(self.TIME_TAG)))
    ...
    def encodeXML(self, msg):
        ...
        if msg.getConversationId():
            sb += self.encodeTag(self.CONVERSATION_ID_TAG, msg.
                getConversationId())
