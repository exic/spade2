...
class Session:
    ...
    def receive(self):
        ...
        #try: received = self._recv(4096)  # Arbitrary value
        try:
            received = self._recv(10240)  # Arbitrary value
        except:
            received = ''
    ...
    def push_queue(self, failreason=ERR_RECIPIENT_UNAVAILABLE):
        ...
        if self._stream_state>=STREAM__CLOSED or self._socket_state>=SOCKET_DEAD: # the stream failed. Return all stanzas that are still waiting for delivery.
            ...
            for key in self.deliver_key_queue:                                          # Not sure. May be I
                self._dispatch(Error(self.
                    deliver_queue_map[key],failreason),trusted=1) # should simply re-dispatch it?
            for stanza in self.stanza_queue:                                            # But such action can invoke
                self._dispatch(Error(stanza, failreason),
                    trusted=1)                      # Infinite loops in case of S2S connection...
    ...
    def flush_queue(self):
        ...
        if self.sendbuffer:
            ...
            self.DEBUG(repr(self.fileno()) + ' ' + self.
    ...
    def set_session_state(self, newstate):
        """ Change the session state.
            ...
            SESSION_AUTHED, SESSION_BOUND,
                 SESSION_OPENED and SESSION_CLOSED states.
        """
