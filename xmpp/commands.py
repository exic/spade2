...
class Commands(PlugIn):
    ...
    def plugin(self, owner):
        ...
        owner.RegisterHandler('iq', self._CommandHandler, typ='set',
            ns=NS_COMMANDS)
        owner.RegisterHandler('iq', self._CommandHandler, typ='get',
            ns=NS_COMMANDS)
    ...
    def plugout(self):
        ...
        self._owner.UnregisterHandler('iq', self_CommandHandler,
             ns=NS_COMMANDS)
    ...
    def _DiscoHandler(self, conn, request, typ):
        ...
        if typ == 'items':
            ...
            if items != []:
                for each in items:
                    i = self._handlers[each[0]][each[1]
                        ]['disco'](conn, request, 'list')
                    if i != None:
                        list.append(Node(tag=
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    'item', attrs={'jid':i[0], 'node':i[1], 'name':i[2]}))
    ...
    def addCommand(self, name, cmddisco, cmdexecute, jid=''):
        ...
        if not jid in self._handlers:
            self._handlers[jid] = {}
            self._browser.setDiscoHandler(self._DiscoHandler,
                node=NS_COMMANDS, jid=jid)
        if name in self._handlers[jid]:
            raise NameError('Command Exists')
        else:
            self._handlers[jid][name] = {'disco': cmddisco,
                 'execute': cmdexecute}
    ...
    def delCommand(self, name, jid=''):
        ...
        if not name in self._handlers[jid]:
    ...
    def getCommand(self, name, jid=''):
        ...
        if not jid in self._handlers:
            raise NameError('Jid not found')
        elif not name in self._handlers[jid]:
            raise NameError('Command not found')
        else:
...
class Command_Handler_Prototype(PlugIn):
    ...
    def __init__(self, jid=''):
        ...
        self.discoinfo = {'ids': [{'category': 'automation', 'type':
            'command-node','name':self.description}],'features': self.discofeatures}
        self._jid = jid
    ...
    def plugin(self, owner):
        ...
        self._commands.addCommand(self.name, self._DiscoHandler,
            self.Execute, jid=self._jid)
...
class TestCommand(Command_Handler_Prototype):
    """ Example class. You should read source if you wish to understate how it works. 
        Generally,
             it presents a "master" that giudes user through to calculate something.
    """
    name = 'testcommand'
    ...
    def cmdFirstStage(self, conn, request):
        ...
        if session == None:
            ...
            sessions[session] = {'jid': request.getFrom(),
                'actions':{'cancel':self.cmdCancel,'next':self.cmdSecondStage},'data':{'type':None}}
        # As this is the first stage we only send a form
        reply = request.buildReply('result')
        form = DataForm(title='Select type of operation', data=[
            'Use the combobox to select the type of calculation you would like to do, then click Next',DataField(name='calctype',label='Calculation Type',value=sessions[session]['data']['type'],options=[['circlediameter','Calculate the Diameter of a circle'],['circlearea','Calculate the area of a circle']],typ='list-single',required=1)])
        replypayload = [Node('actions', attrs={'execute':'next'},
            payload=[Node('next')]), form]
        reply.addChild(name='command', attrs={'xmlns': NS_COMMAND,
            'node':request.getTagAttr('command','node'),'sessionid':session,'status':'executing'},payload=replypayload)
        self._owner.send(reply)
        raise NodeProcessed

    def cmdSecondStage(self, conn, request):
        form = DataForm(node=result.getTag(name='command').
            getTag(name='x', namespace=NS_DATA))
        sessions[request.getTagAttr('command', 'sessionid')]['data']
            ['type'] = form.getField('calctype')
        sessions[request.getTagAttr('command', 'sessionid')][
            'actions']={'cancel':self.cmdCancel,None:self.cmdThirdStage,'previous':cmdFirstStage}
    ...
    def cmdSecondStageReply(self, conn, request):
        reply = request.buildReply('result')
        form = DataForm(title='Enter the radius', data=[
            'Enter the radius of the circle (numbers only)',DataField(label='Radius',name='radius',typ='text-single')])
        replypayload = [Node('actions',attrs={'execute':'complete'}
            , payload=[Node('complete'), Node('prev')]), form]
        reply.addChild(name='command', attrs={'xmlns': NS_COMMAND,
            'node':request.getTagAttr('command','node'),'sessionid':request.getTagAttr('command','sessionid'),'status':'executing'},payload=replypayload)
        self._owner.send(reply)
        raise NodeProcessed
    ...
    def cmdThirdStage(self, conn, request):
        ...
        reply.addChild(name='command', attrs={'xmlns': NS_COMMAND,
            'node':request.getTagAttr('command','node'),'sessionid':request.getTagAttr('command','sessionid'),'status':'completed'},payload=form)
        self._owner.send(reply)
        raise NodeProcessed
    ...
    def cmdCancel(self, conn, request):
        reply = request.buildReply('result')
        reply.addChild(name='command', attrs={'xmlns': NS_COMMAND,
            'node':request.getTagAttr('command','node'),'sessionid':request.getTagAttr('command','sessionid'),'status':'cancelled'})
        self._owner.send(reply)
