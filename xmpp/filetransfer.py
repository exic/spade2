...
class IBB(PlugIn):
    ...
    def __init__(self):
        ...
        self._streams = {}
        self._ampnode = Node(NS_AMP + ' amp', payload=[Node('rule', {
            'condition':'deliver-at','value':'stored','action':'error'}),Node('rule',{'condition':'match-resource','value':'exact','action':'error'})])

    def plugin(self, owner):
        """ Register handlers for receiving incoming datastreams. Used internally. """
        self._owner.RegisterHandlerOnce('iq', self.
            StreamOpenReplyHandler)  # Move to StreamOpen and specify stanza id
...
"""
        err = None
        sid, blocksize = stanza.getTagAttr('open', 'sid'),
            stanza.getTagAttr('open', 'block-size')
        self.DEBUG(
            'StreamOpenHandler called sid->%s blocksize->%s'%(sid,blocksize),'info')
        try:
        ...
        if err:
            ...
            self.DEBUG("Opening stream: id %s, block-size %s" % (
                sid, blocksize), 'info')
            rep = Protocol('iq', stanza.getFrom(), 'result', stanza.
                getTo(), {'id': stanza.getID()})
            self._streams[sid] = {'direction': '<' + str(stanza.
                getFrom()),'block-size':blocksize,'fp':open('/tmp/xmpp_file_'+sid,'w'),'seq':0,'syn_id':stanza.getID()}
        conn.send(rep)
    ...
    def OpenStream(self, sid, to, fp, blocksize=3000):
        ...
        if not JID(to).getResource():
            return
        self._streams[sid] = {'direction': '|>' + to, 'block-size':
            blocksize, 'fp': fp, 'seq': 0}
        self._owner.RegisterCycleHandler(self.SendHandler)
        syn = Protocol('iq', to, 'set', payload=[Node(NS_IBB + ' open', {
            'sid':sid, 'block-size':blocksize})])
    ...
    def SendHandler(self, conn):
        ...
        for sid in self._streams.keys():
            ...
            if stream['direction'][:2] == '|>': cont = 1
                ...
                if chunk:
                    datanode = Node(NS_IBB + ' data', {
                        'sid':sid,'seq':stream['seq']},base64.encodestring(chunk))
                    stream['seq'] += 1
                    if stream['seq'] == 65536:
                        stream['seq'] = 0
                    conn.send(Protocol('message',
                        stream['direction'][1:],payload=[datanode,self._ampnode]))
                else:
                    """ notify the other side about stream closing
                        notify the local user about sucessfull send
                        delete the local stream"""
                    conn.send(Protocol('iq', stream[
                        'direction'][1:],'set',payload=[Node(NS_IBB+' close',{'sid':sid})]))
...
"""
    ...
    def ReceiveHandler(self, conn, stanza):
        ...
        """
        sid, seq, data = stanza.getTagAttr('data', 'sid'),
            stanza.getTagAttr('data', 'seq'), stanza.getTagData('data')
        self.DEBUG('ReceiveHandler called sid->%s seq->%s' % (sid,
             seq), 'info')
        try:
            seq = int(seq)
        ...
        if not sid in self._streams.keys():
            ...
            if not data:
                ...
                self.DEBUG(
                    'Successfull receive sid->%s %s+%s bytes'%(sid,stream['fp'].tell(),len(data)),'ok')
                stream['seq'] += 1
                stream['fp'].write(data)
        if err:
            self.DEBUG('Error on receive: %s' % err, 'error')
            conn.send(Error(Iq(to=stanza.getFrom(), frm=stanza.
                getTo(), payload=[Node(NS_IBB + ' close')]), err, reply=0))
    ...
    def StreamCloseHandler(self, conn, stanza):
        ...
        if sid in self._streams.keys():
            ...
            conn.Event(self.DBG_LINE, 'SUCCESSFULL RECEIVE',
                 self._streams[sid])
            del self._streams[sid]
        else:
        ...
        for sid in self._streams.keys():
            ...
            if stream['syn_id'] == syn_id:
                if stream['direction'][0] == '<':
                    conn.Event(
                    self.DBG_LINE, 'ERROR ON RECEIVE', stream)
                else:
