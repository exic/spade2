...
class TCPsocket(PlugIn):
    ...
    def plugin(self, owner):
        ...
        if not self.connect(self._server):
            return
    ...
    def connect(self, server=None):
        ...
        try:
            ...
            self.DEBUG(
                "Successfully connected to remote host %s" % repr(server), 'start')
            return 'ok'
        except socket.error, (errno, strerror):
            self.DEBUG(
                "Failed to connect to remote host %s: %s (%s)"%(`server`, strerror, errno),'error')
        except:
            pass
...
class HTTPPROXYsocket(TCPsocket):
    ...
    def connect(self, dupe=None):
        ...
        self.DEBUG(
            "Proxy server contacted, performing authentification", 'start')
        ...
        if self._proxy.has_key('user') and self._proxy.has_key('password'):
            credentials = '%s:%s' % (self._proxy['user'], self.
                _proxy['password'])
        ...
        if code != '200':
            self.DEBUG('Invalid proxy reply: %s %s %s' % (proto,
                code, desc), 'error')
            self._owner.disconnected()
            return
        while reply.find('\n\n') == -1:
            try:
                ...
                return
        self.DEBUG(
            "Authentification successfull. Jabber server contacted.", 'ok')
        return 'ok'
...
class TLS(PlugIn):
    ...
    def FeaturesHandler(self, conn, feats):
        ...
        self._owner.RegisterHandlerOnce('proceed', self.
            StartTLSHandler, xmlns=NS_TLS)
        self._owner.RegisterHandlerOnce('failure', self.
            StartTLSHandler, xmlns=NS_TLS)
    ...
    def StartTLSHandler(self, conn, starttls):
        ...
        if self.starttls == 'failure':
            ...
            return
        self.DEBUG(
            "Got starttls proceed response. Switching to TLS/SSL...", 'ok')
        self._startSSL()
