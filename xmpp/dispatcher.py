...
class Dispatcher(PlugIn):
    ...
    def __init__(self):
        ...
        self.RegisterHandlerOnce, self.UnregisterHandler,
    ...
    def _init(self):
        ...
        self.RegisterHandler('error', self.streamErrorHandler,
             xmlns=NS_STREAMS)
    ...
    def StreamInit(self):
        ...
        self._owner.send("<?xml version='1.0'?>%s>" % str(self.
            _metastream)[:-2])
        ...
        if self._owner.Connection.pending_data(timeout):
            ...
            if len(self._pendingExceptions) > 0:
                ...
                raise _pendingException[0]
                    , _pendingException[1], _pendingException[2]
            if data:
                return len(data)
    ...
    def RegisterProtocol(self, tag_name, Proto, xmlns=None, order='info'):
        ...
        self.DEBUG('Registering protocol "%s" as %s(%s)' % (tag_name,
            Proto, xmlns), order)
        self.handlers[xmlns][tag_name] = {type: Proto, 'default': []}

    def RegisterNamespaceHandler(self,xmlns,handler,typ='',ns='', makefirst=0, system=0):
        """ Register handler for processing all stanzas for specified namespace. """
        self.RegisterHandler('default', handler, typ, ns, xmlns,
             makefirst, system)
    ...
    def RegisterHandler(self,name,handler,typ='',ns='',xmlns=None, makefirst=0, system=0):
        ...
        if not xmlns:
            xmlns = self._owner.defaultNamespace
        self.DEBUG(
            'Registering handler %s for "%s" type->%s ns->%s(%s)'%(handler,name,typ,ns,xmlns), 'info')
        if not typ and not ns:
            typ = 'default'
        if not xmlns in self.handlers:
            self.RegisterNamespace(xmlns, 'warn')
        if not name in self.handlers[xmlns]:
            self.RegisterProtocol(name, Protocol, xmlns, 'warn')
        if not (typ + ns) in self.handlers[xmlns][name]:
            self.handlers[xmlns][name][typ + ns] = []
        if makefirst:
            self.handlers[xmlns][name][typ + ns].insert(0, {
    ...
    def UnregisterHandler(self, name, handler, typ='', ns='', xmlns=None):
        ...
        if not typ and not ns:
            typ = 'default'
        if name in self.handlers[xmlns]:
    ...
    def dispatch(self, stanza, session=None, direct=0):
        ...
        if stanza.__class__.__name__ == 'Node':
            stanza = self.handlers[
            xmlns][name][type](node=stanza)
        ...
        session.DEBUG(
            "Dispatching %s stanza with type->%s props->%s id->%s"%(name,typ,stanza.props,ID),'ok')

        list = ['default']
                                                                 # we will use all handlers:
        if typ in self.handlers[xmlns][name]:
            list.append(
            typ)                # from very common...
        for prop in stanza.props:
            if prop in self.handlers[xmlns][name]:
                list.append(prop)
            if typ and self.handlers[xmlns][name].has_key(typ +
                prop):
        ...
        if ID in session._expected:
            ...
            if type(session._expected[ID]) == type(()):
                ...
                session.DEBUG(
                    "Expected stanza arrived. Callback %s(%s) found!"%(cb,args),'ok')
                try:
    ...
    def WaitForResponse(self, ID, timeout=DefaultTimeout):
        ...
        self.DEBUG("Waiting for ID:%s with timeout %s..." % (ID,
            timeout), 'wait')
    ...
    def send(self, stanza):
        ...
        if not isinstance(stanza, Protocol):
            ...
            _ID = stanza.getID()
        if self._owner._registered_name and not stanza.getAttr(
            'from'):
