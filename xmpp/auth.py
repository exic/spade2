...
def H(some):
    return hashlab.md5(some).digest()
...
class NonSASL(PlugIn):
    ...
    def plugin(self, owner):
        ...
        resp = owner.Dispatcher.SendAndWaitForResponse(Iq('get',
            NS_AUTH, payload=[Node('username', payload=[self.user])]))
        ...
        if query.getTag('digest'):
            ...
            query.setTagData('digest', hashlib.sha1(owner.
                Dispatcher.Stream._document_attrs['id']+self.password).hexdigest())
            ...
            hash = hashlib.sha1(hashlib.sha1(self.password).
                hexdigest() + token).hexdigest()
            ...
            self.DEBUG(
                "Sequre methods unsupported, performing plain text authentication",'warn')
    ...
    def authComponent(self, owner):
        ...
        self.handshake = 0
        owner.send(Node(NS_COMPONENT_ACCEPT + ' handshake', payload=[
            hashlib.sha1(owner.Dispatcher.Stream._document_attrs['id']+self.password).hexdigest()]))
...
class SASL(PlugIn):
    ...
    def plugout(self):
        ...
        self._owner.UnregisterHandler('features', self.
    ...
    def FeaturesHandler(self, conn, feats):
        ...
        if "DIGEST-MD5" in mecs:
            ...
            sasl_data = '%s\x00%s\x00%s' % (self.username + '@' + self.
                _owner.Server, self.username, self.password)
            node = Node('auth', attrs={'xmlns': NS_SASL,
...
########################################3333
        ...
        if 'qop' in chal and chal['qop'] == 'auth':
            ...
            A1 = C([H(C([resp['username'], resp['realm'], self.
                password])), resp['nonce'], resp['cnonce']])
            A2 = C(['AUTHENTICATE', resp['digest-uri']])
            response = HH(C([HH(A1), resp['nonce'], resp['nc'],
                resp['cnonce'], resp['qop'], HH(A2)]))
...
########################################3333
            ...
            self.startsasl = 'failure'
            self.DEBUG(
                'Failed SASL authentification: unknown challenge', 'error')
        raise NodeProcessed
...
class Bind(PlugIn):
    ...
    def Bind(self, resource=None):
        ...
        if resource:
            ...
            resource = []
        resp = self._owner.SendAndWaitForResponse(Protocol('iq',
             typ=            'set',payload=[Node('bind',attrs={'xmlns':NS_BIND},payload=resource)]))
        if isResultNode(resp):
            ...
            resp = self._owner.SendAndWaitForResponse(Protocol(
                'iq',typ='set',payload=[Node('session',attrs={'xmlns':NS_SESSION})]))
            if isResultNode(resp):
...
class ComponentBind(PlugIn):
    ...
    def plugin(self, owner):
        ...
        if self._owner.Dispatcher.Stream.features:
            ...
            self._owner.RegisterHandler('features', self.
                FeaturesHandler, xmlns=NS_STREAMS)
    ...
    def Bind(self, domain=None):
        ...
        while self.bound is None and self._owner.Process(1):
            pass
        resp = self._owner.SendAndWaitForResponse(Protocol('bind',
            attrs={'name': domain}, xmlns=NS_COMPONENT_1))
