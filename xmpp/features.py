...
def _discover(disp, ns, jid, node=None, fb2b=0, fb2a=1):
    ...
    if fb2b and not isResultNode(rep):
        rep = disp.SendAndWaitForResponse(
        Iq(to=jid, typ='get', queryNS=NS_BROWSE))   # Fallback to browse
    if fb2a and not isResultNode(rep):
        rep = disp.SendAndWaitForResponse(
        Iq(to=jid, typ='get', queryNS=NS_AGENTS))   # Fallback to agents
    if isResultNode(rep):
        return rep.getQueryPayload()
    return []
...
def _ReceivedRegInfo(con, resp, agent):
    ...
    if not isResultNode(resp):
        return
    df = resp.getTag('query', namespace=NS_REGISTER).getTag('x',
         namespace=NS_DATA)
    if df:
        con.Event(NS_REGISTER, REGISTER_DATA_RECEIVED, (agent,
             DataForm(node=df)))
        return
...
def changePasswordTo(disp, newpassword, host=None):
    ...
    resp = disp.SendAndWaitForResponse(Iq('set', NS_REGISTER, to=host,
        payload=[Node('username',payload=[disp._owner.Server]),Node('password',payload=[newpassword])]))
    if isResultNode(resp):
        return 1
...
def getPrivacyList(disp, listname):
    ...
    try:
        resp = disp.SendAndWaitForResponse(Iq('get', NS_PRIVACY,
            payload=[Node('list', {'name':listname})]))
        if isResultNode(resp):
...
def setActivePrivacyList(disp, listname=None, typ='active'):
    ...
    if listname:
        ...
        attrs = {}
    resp = disp.SendAndWaitForResponse(Iq('set', NS_PRIVACY, payload=[Node(
        typ, attrs)]))
    if isResultNode(resp):
        return 1
