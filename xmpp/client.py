...
# $Id: client.py,v 1.54 2006/02/11 15:37:09 snakeru Exp $
    ...
    def PlugIn(self, owner):
        ...
        for method in self._exported_methods:
            if method.__name__ in owner.__dict__:
                self._old_owners_methods.append(owner.
                    __dict__[method.__name__])
...
class CommonClient:
    ...
    def connect(self, server=None, proxy=None, ssl=None, use_srv=None):
        ...
        if proxy:
            socket = transports.HTTPPROXYsocket(proxy, server, use_srv)
        else:
            socket = transports.TCPsocket(server, use_srv)
...
#    def connect(self,server=None,proxy=None,secure=None,use_srv=True):
    def connect(self, server=None, proxy=None, secure=None, use_srv=None):
        ...
        while not self.Dispatcher.Stream.features and self.Process(
            1): pass      # If we get version 1.0 stream the features tag MUST BE presented
        ...
        while not self.TLS.starttls and self.Process(1):
            pass
        if not hasattr(self,
             'TLS') or self.TLS.starttls != 'success':
    ...
    def getRoster(self):
        ...
        if not 'Roster' in self.__dict__:
