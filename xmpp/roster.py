...
class Roster(PlugIn):
    ...
    def plugin(self, owner, request=1):
        """ Register presence and subscription trackers in the owner's dispatcher.
            ...
            Used internally."""
        self._owner.RegisterHandler('iq', self.RosterIqHandler,
            'result', NS_ROSTER)
        self._owner.RegisterHandler('iq', self.RosterIqHandler,
             'set', NS_ROSTER)
    ...
    def PresenceHandler(self, dis, pres):
        ...
        if not self._data.has_key(jid.getStripped()):
            self._data[
            jid.getStripped()]={'name':None,'ask':None,'subscription':'none','groups':['Not in roster'],'resources':{}}
        ...
        if not typ:
            self.DEBUG(
                'Setting roster item %s for resource %s...'%(jid.getStripped(),jid.getResource()),'ok')
            item['resources'][jid.getResource()] = res = {'show':
                None, 'status': None, 'priority': '0', 'timestamp': None}
            if pres.getTag('show'):
    ...
    def _getResourceData(self, jid, dataname):
        ...
        if jid.find('/') + 1:
            ...
            for r in self._data[jid]['resources'].keys():
                if int(self._data[jid]['resources'][r][
                    'priority'])>lastpri: resource,lastpri=r,int(self._data[jid]['resources'][r]['priority'])
