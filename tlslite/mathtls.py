...
def PRF(secret, label, seed, length):
    ...
    p_md5 = P_hash(hashlib, S1, concatArrays(stringToBytes(label),
         seed), length)
    p_sha1 = P_hash(hashlib, S2, concatArrays(stringToBytes(label),
         seed), length)
...
def makeX(salt, username, password):
    ...
    return stringToNumber(hashlib.sha1(salt + hashlib.sha1(username + ":" + password)\
           .digest()).digest())
...
def makeVerifier(username, password, bits):
    bitsIndex = {1024: 0, 1536: 1, 2048: 2, 3072: 3, 4096: 4, 6144: 5,
         8192: 6}[bits]
